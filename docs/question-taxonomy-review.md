...
A canonical question with separate, individually unambiguous RN and PN source collections can additionally return `shared_classification_allowed: true`, `shared_nclex_variants: ["nclex_pn", "nclex_rn"]`, and `shared_allowed_systems`. After reviewing its full evidence and the appropriateness of the same four-level path for both variants, submit `nclex_variants: ["nclex_pn", "nclex_rn"]` instead of `nclex_variant`. A single shared path must use the exact intersection of both diagnostic blueprints. Management/Coordinated Care and the differing pharmacology categories cannot be forced into one common path. Lower clinical labels still require explicit review for both audiences.
...
### Different reviewed paths for RN and PN

Eligible shared exports also include `shared_variant_allowed_systems`, with each variant's complete allowed root list, and `shared_source_bindings`. For questions requiring different categories, retain the explicit `nclex_variants` pair and supply a taxonomy object with `kind: "nclex_variant_paths_v1"`, `paths: { nclex_rn: <complete four-level taxonomy>, nclex_pn: <complete four-level taxonomy> }`, and `source_bindings` copied exactly from the export. Each path is validated against its own variant's roots. The review must substantively cover both paths; structural validity is not clinical approval.

Bindings include variant, collection UUID, collection title/key/provider/profile, alias namespace, item ID and import filename. Both variants and every registered source binding must match the exported evidence. Partial, duplicated, fabricated or stale bindings are rejected. A collection cannot bind both variants. The stored representation has no default RN or PN path: it retains both paths, the original broad fallback taxonomy and the existing override/review identity.

Student facets, exact selection counts, catalog summaries and question delivery project the path from the actual approved source alias chosen by the existing access rules. The alias's current identity and collection metadata must still exactly match a reviewed binding. A new or changed source falls back to the saved pre-review taxonomy until re-reviewed; it does not inherit another bank's reviewed category. The student question projection contains only the chosen clinical taxonomy and review metadata, without source bindings or the alternate path. This adds no schema migration or new access rule. Catalog and delivery use the same deterministic source preference when multiple permitted sources are present.
