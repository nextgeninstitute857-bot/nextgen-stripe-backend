...
test("classification request requires web search and a strict review schema", () => {
  ...
  assert.match(request.systemPrompt, /No Vimeo caption transcript is available/);
  assert.match(request.systemPrompt, /untrusted content/);
  assert.match(request.systemPrompt, /owner is not a doctor/i);
  assert.match(request.systemPrompt, /compatibility names/i);
});

test("Vimeo VTT captions become compact lecture evidence without timestamps or markup", () => {
  const transcript = stripVimeoVtt(`WEBVTT

00:00:00.000 --> 00:00:02.000
<v Speaker>Acute coronary syndrome</v>
...
`);
  assert.equal(
    transcript,
    "Acute coronary syndrome includes unstable angina & myocardial infarction.",
  );
});

test("Vimeo lecture evidence selects the active English caption track and fingerprints only its cleaned text", async () => {
  const calls = [];
  const evidence = await fetchVimeoLectureEvidence({
    videoId: "123456",
    apiClient: {
      get: async (url) => {
        calls.push(url);
        return {
          data: {
            data: [
              { uri: "/videos/123456/texttracks/1", name: "Spanish", language: "es", active: true, link: "https://captions.example/es.vtt" },
              { uri: "/videos/123456/texttracks/2", name: "auto_generated_captions.vtt", language: "en-x-autogen", active: true, type: "subtitles", link: "https://captions.example/en.vtt" },
            ],
          },
        };
      },
  ...
  assert.deepEqual(calls, [
    "/videos/123456/texttracks",
    "https://captions.example/en.vtt",
  ]);
  assert.equal(evidence.available, true);
  assert.equal(evidence.transcript, "Acute coronary syndrome");
  assert.equal(evidence.language, "en-x-autogen");
  assert.equal(evidence.autoGenerated, true);
  assert.match(evidence.transcriptFingerprint, /^[a-f0-9]{64}$/);
});

test("caption transcript is supplied to classification but never marked for persistence", () => {
  const request = buildVimeoTopicClassificationRequest(catalogDraft(), {
    examTrackLabel: "USMLE Step 1",
    allowedSystems: ["Cardiovascular", "Renal"],
    taxonomyRows: taxonomyRows(),
    contentEvidence: {
      ...
      transcript: "This lecture repeatedly covers acute coronary syndrome, unstable angina, and myocardial infarction.",
      transcriptFingerprint: "a".repeat(64),
      wordCount: 12,
      characterCount: 108,
      trackUri: "/videos/123456/texttracks/2",
      trackName: "auto_generated_captions.vtt",
      language: "en-x-autogen",
      autoGenerated: true,
    },
  });
  assert.match(request.systemPrompt, /caption transcript is supplied as direct lecture-content evidence/i);
  assert.match(request.userPrompt, /unstable angina/);
  assert.equal(request.contentEvidence.available, true);
  assert.equal(request.contentEvidence.transcriptFingerprint, "a".repeat(64));
  assert.equal(Object.hasOwn(request.contentEvidence, "transcript"), false);
});
...
test("server wiring is draft-first, background researched, and explicit-review only", () => {
  ...
  assert.match(server, /app\.put\("\/api\/ayla\/admin\/resources\/vimeo-delivery-controls"/);
  assert.match(server, /app\.post\("\/api\/ayla\/admin\/resources\/vimeo-catalog\/permanent-removal"/);
  assert.match(server, /PERMANENTLY REMOVE \$\{videoId\}/);
  assert.match(server, /forNotes: true/);
