...
test("the registry persists source year and applies provider-scoped delivery years", () => {
  ...
  assert.match(registry, /function contentCollectionDeliveryYearReadySql/);
  assert.match(registry, /function contentCollectionIsUworldSql/);
  assert.match(registry, /CONTENT_DELIVERY_SOURCE_YEARS\.join\(","\)/);
  assert.match(registry, /CONTENT_DELIVERY_VERIFIED_FALLBACK_SOURCE_YEARS\.join\(","\)/);
...
test("QBank catalog, session creation, roadmap selection, diagnostics, and flashcards share the policy", () => {
  ...
  assert.match(server, /!contentDeliveryRecordAllowed\(question\)/);
