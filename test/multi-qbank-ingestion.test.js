...
async function waitFor(check, timeoutMs = 3_000) {
  ...
  throw new Error("Timed out waiting for v238 state");
}

test("v238 defaults to two QBank media lanes with bounded shared capacity", () => {
  assert.deepEqual(multiQbankIngestionConfig({}), {
    build: "v238-qbank-bulk-ingestion",
    max_active_jobs: 4,
    lane_concurrency: {
      question_zip: 2,
      image_zip: 2,
...
test("v238 server wiring keeps binaries private and serializes only finalization", async () => {
