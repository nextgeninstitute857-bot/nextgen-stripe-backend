...
/** Draft only the declared writable collections; other data is read-only.
 * This contract is for bounded operations that scan large content collections
 * but update only a few state collections. It avoids creating content proxies.
 */
export async function mutateJsonCollectionsCopyOnWrite(source, collections, mutator) {
  const writable = Object.fromEntries(collections.map(key => [key, source[key] || {}]));
  const mutation = await mutateJsonCopyOnWrite(writable, async draft => {
    ...
    for (const key of collections) {
      Object.defineProperty(view, key, {
        enumerable: true,
        get: () => draft[key],
        set: value => { draft[key] = value; },
      });
    }
    // Reject top-level writes outside the declared scope. Nested content is
    // deliberately unproxied and must remain read-only in the mutator.
    Object.freeze(view);
    return mutator(view);
  });
  return { ...mutation, value: mutation.changed ? { ...source, ...mutation.value } : source };
}
