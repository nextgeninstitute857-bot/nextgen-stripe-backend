...
export const AYLA_PUBLICATION_RESOURCE_TYPES = Object.freeze([
  ...
  "flashcard_collection", "study_program",
]);

export const AYLA_PUBLICATION_GROUP_ORDER = Object.freeze([
  "qbank_collection", "vimeo_folder", "book_folder", "flashcard_collection", "study_program",
]);
...
export function aylaQbankCollectionDeliveryChannel(collection = {}, {
  hasQbankDestination = false,
  hasNbmeDestination = false,
} = {}) {
  const examTrack = key(collection.exam_track ?? collection.examTrack);
  const sourceNamespace = clean(collection.source_namespace ?? collection.sourceNamespace, 180).toLowerCase();
  const collectionIdentity = `${collection.collection_key || collection.collectionKey || ""} ${collection.title || ""}`.toLowerCase();
  const fingerprint = `${sourceNamespace} ${collectionIdentity}`;
  const knownStep1TestNamespace = examTrack === "usmle_step_1"
    && new Set(["step-1-simulation", "step-1-sim", "sim-1-march", "simuladtion-1"]).has(sourceNamespace);
  if (knownStep1TestNamespace || /aylamed_step1_cross_system_preview|internal preview/.test(fingerprint)) {
    return "internal_testing";
  }
  const retainedStep1Simulation = examTrack === "usmle_step_1"
    && sourceNamespace === "uworld-step-1"
    && Number(collection.source_year ?? collection.sourceYear ?? 0) === 2026
    && /(?:^|[-_\s])sim(?:ulation)?[-_\s]?[123](?:$|[-_\s])/.test(collectionIdentity);
  if (retainedStep1Simulation) return "assessment";
  return hasNbmeDestination && !hasQbankDestination ? "nbme" : "qbank";
}
...
function publicationDestinationKey(value = "") {
  const normalized = String(value || "").trim().toLowerCase()
    .replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
  const aliases = {
    aylamed_qbank: "qbank",
    aylamed_content_hub: "content_hub",
    aylamed_roadmap: "roadmap",
  };
  return aliases[normalized] || normalized;
}

function qbankProviderLabel(resource = {}) {
  const profile = key(resource.source_profile ?? resource.sourceProfile);
  const provider = clean(resource.source_provider ?? resource.sourceProvider ?? resource.provider, 180);
  const fingerprint = `${profile} ${provider} ${resource.source_namespace || ""}`.toLowerCase();
  if (/uworld/.test(fingerprint)) return "UWorld";
  if (/amboss/.test(fingerprint)) return "AMBOSS";
  if (/canada\s*qbank|canadaqbank/.test(fingerprint)) return "CanadaQBank";
  if (/ace\s*qbank|aceqbank/.test(fingerprint)) return "ACE QBank";
  if (/board\s*vitals|boardvitals/.test(fingerprint)) return "BoardVitals";
  if (/pass\s*medicine|passmedicine/.test(fingerprint)) return "PassMedicine";
  if (/bmj|onexamination/.test(fingerprint)) return "BMJ OnExamination";
  if (/amedex/.test(fingerprint)) return "Amedex";
  if (/mplusx|mplus\s*x/.test(fingerprint)) return "MPlusX";
  if (/internal|interrnal|simulation/.test(fingerprint)) return "Internal simulations";
  if (/aylamed/.test(fingerprint)) return "AylaMed";
  return provider || publicationGroupTitle(resource, "Named question bank");
}

function qbankFolder(resource = {}) {
  const id = clean(resource.source_namespace ?? resource.sourceNamespace
    ?? resource.folder_id ?? resource.folderId ?? resource.collection_key ?? resource.collectionKey, 300)
    || `unfiled-${compactId(qbankProviderLabel(resource))}`;
  return { id, title: clean(resource.folder_name ?? resource.folderName, 300) || id };
}
...
export function aylaPublicationReadinessBlockers(resource = {}, identity = aylaPublicationGroupForResource(resource)) {
  ...
  if (identity?.groupType === "qbank_collection") {
    ...
    const deliveryReady = optionalNumberFrom(resource, "delivery_media_ready_count", "deliveryMediaReadyCount");
    ...
    if (questions && deliveryReady !== null && deliveryReady < 1) blockers.push("No questions pass required-media delivery checks");
  }
...
export function aylaPublicationGroupForResource(resource = {}) {
  ...
  if (resourceType === "qbank_collection") {
    const deliveryChannel = key(resource.delivery_channel ?? resource.deliveryChannel);
    if (deliveryChannel && deliveryChannel !== "qbank_collection") return null;
    const bankName = qbankProviderLabel(resource);
    const folder = qbankFolder(resource);
    return {
      groupType: "qbank_collection",
      groupId: `bank-${compactId(bankName)}`,
      title: bankName,
      bankName,
      folderId: folder.id,
...
function configuredPublicationState(examTrackId, groupType, groupId, controls = [], fallbackEnabled = true) {
  ...
  return { enabled: control ? control.enabled : fallbackEnabled, inherited: !control, control: control || null };
}
...
export function buildAylaCompactPublicationGroups({ exams = [], availableResources = [], resourceControls = [] } = {}) {
  ...
  for (const resource of rows(availableResources)) {
    ...
    const existing = groups.get(groupKey) || {
      ...
      resource_count: 0,
      question_count: 0,
      taxonomy_complete_count: 0,
      delivery_media_ready_count: 0,
      delivery_media_ready_count_known: true,
      video_count: 0,
      ...
      readiness_blockers: [],
      bank_name: identity.bankName || null,
      folders: [],
    };
    if (!existing.resources.some((row) => row.id === String(resource.id || "") && row.type === key(resource.type))) {
      existing.resources.push({
        ...
        source_namespace: clean(resource.source_namespace || resource.sourceNamespace, 300),
        source_provider: clean(resource.source_provider || resource.sourceProvider, 180),
        source_year: optionalNumberFrom(resource, "source_year", "sourceYear"),
        question_count: numberFrom(resource, "question_count", "questionCount"),
        taxonomy_complete_count: numberFrom(resource, "taxonomy_complete_count", "taxonomyCompleteCount"),
        delivery_media_ready_count: optionalNumberFrom(resource, "delivery_media_ready_count", "deliveryMediaReadyCount"),
        publication_default_enabled: resource.publication_default_enabled !== false,
      });
    }
    existing.resource_count = existing.resources.length;
    existing.question_count += Number(resource.question_count || 0);
    existing.taxonomy_complete_count = Number(existing.taxonomy_complete_count || 0) + Number(resource.taxonomy_complete_count || 0);
    const deliveryReady = optionalNumberFrom(resource, "delivery_media_ready_count", "deliveryMediaReadyCount");
    if (deliveryReady === null) existing.delivery_media_ready_count_known = false;
    else existing.delivery_media_ready_count += deliveryReady;
    if (identity.folderId) {
      let folder = existing.folders.find((row) => row.id === identity.folderId);
      if (!folder) {
        folder = { id: identity.folderId, title: identity.folderTitle || identity.folderId, collection_count: 0, question_count: 0 };
        existing.folders.push(folder);
      }
      folder.collection_count += 1;
      folder.question_count += Number(resource.question_count || 0);
    }
  ...
  return [...groups.values()].map((group) => {
    ...
    const memberStates = group.resources.map((resource) => configuredPublicationState(
      ...
      resource.id,
      resourceControls,
      resource.publication_default_enabled !== false,
    ).enabled);
    const groupControlWins = !publication.inherited;
    const mixed = !groupControlWins && new Set(memberStates).size > 1;
    const configuredEnabled = groupControlWins
      ? publication.enabled
      : memberStates.length ? memberStates.every(Boolean) : publication.enabled;
    ...
    return {
      ...
      configured_state: mixed ? "mixed" : configuredEnabled ? "published" : "unpublished",
      configured_enabled: configuredEnabled,
      inherited: publication.inherited && group.resources.every((resource) => configuredPublicationState(
        group.exam_track_id, resource.type, resource.id, resourceControls,
        resource.publication_default_enabled !== false,
      ).inherited),
      mixed_state: mixed,
      effective_student_access: examEnabled && configuredEnabled && !mixed,
      effective_state: mixed ? "mixed_review_required" : examEnabled && configuredEnabled ? "available" : "unavailable",
      control_id: publication.control?.id || null,
      excluded_question_count: group.delivery_media_ready_count_known
        ? Math.max(0, Number(group.question_count || 0) - Number(group.delivery_media_ready_count || 0))
        : null,
      folders: group.folders.sort((left, right) => left.title.localeCompare(right.title)),
    };
  }).sort((left, right) => {
...
export function resolveAylaExamPublication({
  ...
  const destinationKey = publicationDestinationKey(destination);
