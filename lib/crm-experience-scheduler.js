...
export async function runExperienceCheckin({ leadId, read, mutate, context, generate, send, review, lock, unlock, now = Date.now }) {
  ...
  try {
    ...
    const reserved = await mutate(async (db) => {
      const current = context(db, leadId);
      const check = eligibility(current);
      if (!check.ok || check.item.id !== decision.item.id || fingerprint(current) !== fingerprint(initial)) return false;
    ...
    return await mutate(async (db) => {
      ...
      if (!check.ok || check.item.id !== item.id || fingerprint(current) !== fingerprint(initial)) {
      ...
      let result;
      try {
        result = await send({ db, lead: current.lead, item, reply });
      } catch (error) {
