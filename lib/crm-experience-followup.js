...
export function experienceWaitHours(value) {
  ...
  return Number.isFinite(hours) && hours > 0 ? Math.max(6, Math.min(72, hours)) : 18;
}
...
export function experienceResourcesFromDelivery({ snapshot = {}, results = [], demoDays = 7, templateDefinitions = [] } = {}) {
  ...
  const resources = candidates.filter((item) => clean(item.title) && item.url && urls.has(item.url)
...
export function recordExperienceShares({ lead, resources = [], inbound = {}, now = new Date().toISOString(), waitHours = 18 } = {}) {
  if (!lead || !at(now)) return [];
  const items = Array.isArray(lead.ayla_experience_followups) ? lead.ayla_experience_followups : [];
  const added = [];
  for (const resource of resources) {
    if (!["demo", "recording", "live_session"].includes(resource.kind) || !clean(resource.title) || !/^https:\/\//i.test(resource.url || "")) continue;
    const key = createHash("sha256").update(JSON.stringify([resource.kind, resource.url, resource.title, resource.kind === "live_session" ? resource.starts_at || resource.date || "" : ""])).digest("hex").slice(0, 24);
    ...
    const due = resource.kind === "live_session" && !start ? null
...
export function experienceFollowupEligibility({ lead = {}, latestInbound = {}, latestOutbound = {}, futureFollowups = [], now = Date.now(), channel = "whatsapp", reservationId = null } = {}) {
  ...
  const item = all.filter((row) => ["pending", "template_required"].includes(row.status) && !row.checkin_sent_at && row.channel === channel)
    .sort((a, b) => at(b.shared_at) - at(a.shared_at) || (a.kind === "demo" ? 1 : 0) - (b.kind === "demo" ? 1 : 0))[0];
  ...
  if (now - messageTime(latestInbound) >= 24 * hour) return fail("experience_template_required");
  return { ok: true, reason: "ready", item };
}

export function buildExperienceCheckinPrompt({ item, name = "", messages = [] } = {}) {
  return `Write one warm, concise WhatsApp check-in as Ayla from NextGen. The student was sent this exact resource, but we DO NOT know whether they used it.
