...
export class SafeBackgroundQueue {
  ...
  async initialize() {
    ...
    this.initializing = (async () => {
      ...
      try {
        parsed = JSON.parse(await fs.readFile(this.manifestFile, "utf8"));
      } catch (error) {
      ...
      if (this.persistentStore?.load) {
        const storedRows = await this.persistentStore.load();
        for (const row of Array.isArray(storedRows) ? storedRows : []) {
          if (!row?.id || !row?.type) continue;
          const existing = recoveredRows.get(String(row.id));
          const existingUpdated = Date.parse(existing?.updated_at || 0);
          const storedUpdated = Date.parse(row.updated_at || 0);
          if (!existing || !Number.isFinite(existingUpdated) || storedUpdated >= existingUpdated) {
            recoveredRows.set(String(row.id), row);
          }
        }
      }
      ...
      if (changed || !Array.isArray(parsed.jobs)) await this.persist(
        changed ? [...this.jobs.values()] : [],
      );
