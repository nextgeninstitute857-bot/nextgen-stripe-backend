...
export async function ensureVimeoEmbedDomains({
  ...
  onProgress,
  rateLimitAttempts,
  secureEmbedOnly = false,
  allowEmbedOnlyPublicFallback = false,
  adapters = {},
} = {}) {
  ...
  for (const videoId of cleanVideoIds) {
    ...
    try {
      ...
      const currentEmbedPrivacy = String(metadata?.privacy?.embed || "").toLowerCase();
      const currentViewPrivacy = String(metadata?.privacy?.view || "").toLowerCase();
      if (secureEmbedOnly
        && allowEmbedOnlyPublicFallback
        && currentViewPrivacy === "nobody") {
        await withProviderRateLimitBackoff(
          () => api.patch(`/videos/${videoId}`, {
            privacy: { view: "disable", embed: "public" },
          }),
          retryOptions,
        );
        const directFallbackResponse = await withProviderRateLimitBackoff(
          () => api.get(`/videos/${videoId}`, {
            params: { fields: "uri,link,player_embed_url,privacy" },
          }),
          retryOptions,
        );
        const directFallbackMetadata = vimeoResponseData(directFallbackResponse);
        const directFallbackEmbed = String(
          directFallbackMetadata?.privacy?.embed || "",
        ).toLowerCase();
        const directFallbackView = String(
          directFallbackMetadata?.privacy?.view || "",
        ).toLowerCase();
        if (directFallbackEmbed !== "public" || directFallbackView !== "disable") {
          throw Object.assign(new Error("Vimeo did not confirm secure embed-only playback"), {
            statusCode: 409,
          });
        }
        result.privacy_mode_updates += 1;
        result.embed_only_view_updates += 1;
        result.public_embed_fallback_videos.push(videoId);
        result.video_configs.push({
          video_id: videoId,
          embed_url: vimeoEmbedUrl(videoId, directFallbackMetadata),
          privacy_embed: directFallbackEmbed,
          privacy_view: directFallbackView,
          allowed_domains: [],
      ...
      const privacyUpdate = {
        ...(currentEmbedPrivacy !== "whitelist" ? { embed: "whitelist" } : {}),
        ...(secureEmbedOnly && currentViewPrivacy !== "disable" ? { view: "disable" } : {}),
      };
      if (Object.keys(privacyUpdate).length) {
        await withProviderRateLimitBackoff(
          () => api.patch(`/videos/${videoId}`, { privacy: privacyUpdate }),
          retryOptions,
        );
        result.privacy_mode_updates += 1;
        if (privacyUpdate.view === "disable") result.embed_only_view_updates += 1;
      }
    } catch (error) {
    ...
    try {
      ...
      let refreshedMetadata = vimeoResponseData(refreshed);
      let privacyEmbed = String(
        refreshedMetadata?.privacy?.embed || metadata?.privacy?.embed || "",
      ).toLowerCase();
      let privacyView = String(
        refreshedMetadata?.privacy?.view || metadata?.privacy?.view || "",
      ).toLowerCase();
      let fallbackMode = null;
      const videoHasFailures = result.failures.some((row) =>
        String(row.video_id || "") === videoId);
      if (allowEmbedOnlyPublicFallback && videoHasFailures) {
        try {
          await withProviderRateLimitBackoff(
            () => api.patch(`/videos/${videoId}`, {
              privacy: { view: "disable", embed: "public" },
          ...
          const fallbackResponse = await withProviderRateLimitBackoff(
            () => api.get(`/videos/${videoId}`, {
              params: { fields: "uri,link,player_embed_url,privacy" },
            }),
            retryOptions,
          );
          const fallbackMetadata = vimeoResponseData(fallbackResponse);
          const fallbackEmbed = String(fallbackMetadata?.privacy?.embed || "").toLowerCase();
          const fallbackView = String(fallbackMetadata?.privacy?.view || "").toLowerCase();
          if (fallbackEmbed === "public" && fallbackView === "disable") {
            result.failures = result.failures.filter((row) =>
              String(row.video_id || "") !== videoId);
            result.public_embed_fallback_videos.push(videoId);
            refreshedMetadata = fallbackMetadata;
            privacyEmbed = fallbackEmbed;
            privacyView = fallbackView;
            fallbackMode = "secure_embed_only_public";
          }
        } catch (error) {
          result.failures.push({
            video_id: videoId,
            domain: null,
            status: Number(error?.response?.status || error?.statusCode || 0) || null,
            error: `Vimeo secure embed-only fallback failed: ${String(error?.response?.data?.error || error?.message || error)}`.slice(0, 500),
          });
        }
      }
      if (!fallbackMode && privacyEmbed !== "whitelist") {
        result.failures.push({
          video_id: videoId,
          domain: null,
          status: 409,
