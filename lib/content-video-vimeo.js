...
export async function extractReferencedVideos({ zipFile, zipSource = zipFile, references = [], onProgress }) {
  ...
  await new Promise((resolve, reject) => {
    zip.on("entry", async (entry) => {
      try {
        ...
        if (!originalName || !mediaMatchKeys(originalName).some((key) => keys.has(key))) {
          const now = Date.now();
...
export async function uploadVideoToVimeo({ file, stream, sizeBytes, name, description = "", onProgress }) {
  ...
  hardTimer.unref?.();
  try {
    await axios.patch(uploadLink, body, {
      headers: {
        "Tus-Resumable": "1.0.0",
        "Upload-Offset": "0",
