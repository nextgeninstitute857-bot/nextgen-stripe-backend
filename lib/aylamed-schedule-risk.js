...
export function buildAylaScheduleRisk(input = {}) {
  ...
  const recoveryExtraDailyMinutes = backlogMinutes ? Math.max(1, Math.ceil(backlogMinutes / studyDaysRemaining)) : 0;
  const capacityExtraDailyMinutes = Math.max(0, requiredDailyMinutes - dailyCapacityMinutes);
  const extraDailyMinutes = Math.max(recoveryExtraDailyMinutes, capacityExtraDailyMinutes);
  ...
  if (hasRisk) {
    ...
    const recovery = extraDailyMinutes
