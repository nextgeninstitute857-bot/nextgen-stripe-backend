...
function checkpoint(day, state, tutorDecision, systems) {
  ...
  const expectedWeakestSystem = [...progress]
    .sort((left, right) => left.masteryPercent - right.masteryPercent || left.system.localeCompare(right.system))[0]?.system || null;
  ...
  return {
    ...
    acceptance: {
      ...
      personalTutorTracksWeakestSystem: Object.values(state.checkpoints).every((row) => (
        Boolean(row.tutorWeakSystem) && row.tutorWeakSystem === row.expectedWeakestSystem
      )),
