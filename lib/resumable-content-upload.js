...
const PURPOSES = new Set(["question_zip", "image_zip", "video_zip"]);
...
function cleanFilename(value) {
  ...
  if (!/\.zip$/i.test(clean)) throw statusError("Only ZIP uploads are accepted", 400);
  return clean;
}
...
export class ResumableContentUploadStore {
  ...
  finalFile(id) {
    return path.join(this.sessionDir(id), "final.zip");
  }
  ...
  async create({ originalFilename, totalBytes, expectedSha256 = "", purpose = "question_zip", createdBy = "", metadata = {} } = {}) {
    ...
    if (bytes > this.maxUploadBytes) throw statusError("ZIP exceeds the configured upload limit", 413);
    const cleanPurpose = String(purpose || "question_zip").trim().toLowerCase();
    if (!PURPOSES.has(cleanPurpose)) throw statusError("purpose must be question_zip, image_zip, or video_zip", 400);
  ...
  async finalize(id, { expectedSha256 = "" } = {}) {
    ...
    return this.withLock(id, async () => {
      ...
      if (session.status === "finalized") {
        await fs.access(this.finalFile(id)).catch(() => { throw statusError("Finalized ZIP file is missing", 410); });
        return { session: publicSession(session), file: this.finalFile(id), sha256: session.final_sha256, deduplicated: true };
      }
      ...
      const finalPath = this.finalFile(id);
      ...
      try {
        ...
        if (requestedSha && digest !== requestedSha) throw statusError("Final ZIP SHA-256 does not match", 409);
  ...
  async resolveFinalized(id, { allowedPurposes = [] } = {}) {
    return this.withLock(id, async () => {
      ...
      if (this.now().getTime() > Date.parse(session.expires_at)) throw statusError("Upload session has expired", 410);
      if (allowedPurposes.length && !allowedPurposes.includes(session.purpose)) throw statusError("Upload purpose does not match this import", 409);
      const file = this.finalFile(id);
      await fs.access(file).catch(() => { throw statusError("Finalized ZIP file is no longer available", 410); });
      session.updated_at = this.now().toISOString();
      session.expires_at = new Date(this.now().getTime() + this.sessionTtlMs).toISOString();
      await this.write(session);
      return {
