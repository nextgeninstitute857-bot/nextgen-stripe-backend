...
const NEXTGEN_BRAND_ID = "brand_nextgen_usmle";
...
export const NEXTGEN_WHATSAPP_TEMPLATE_PACK = Object.freeze([
  ...
  },
  {
    ...
    body: "Hi Dr. {{name}} 👋 {{topic}} is starting now. Here’s your class link: {{live_session_link}} See you in class.",
  ...
  },
  {
    ...
    body: "Hi Dr. {{name}} 👋 Your recording and notes for {{full_session_name}} are now ready in your NextGen LMS account. Open them here: {{recording_link}} whenever you’re ready.",
...
export function reconcileNextGenWhatsAppTemplatePack(records = [], options = {}) {
  ...
  const untouchedOtherBrands = source.filter((record) => (
    String(record?.brand_id || "").trim()
    && String(record.brand_id) !== NEXTGEN_BRAND_ID
  ));
  const next = source
    .filter((record) => !String(record?.brand_id || "").trim() || String(record.brand_id) === NEXTGEN_BRAND_ID)
    .map((record) => ({ ...record }));
  const archived = [];
  ...
  return {
    templates: [...next, ...untouchedOtherBrands],
