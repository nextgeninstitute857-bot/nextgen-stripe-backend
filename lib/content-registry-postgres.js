...
export async function getContentMediaReferences(contentImportJobId) {
  await ensureContentRegistrySchema();
  ...
  return result.rows.map((row) => ({ questionId: row.question_id, studentQid: row.student_qid, mediaRef: row.media_ref }));
}
