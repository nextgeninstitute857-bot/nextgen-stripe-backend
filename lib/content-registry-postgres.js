...
const CONTENT_DELIVERY_YEAR_SQL = CONTENT_DELIVERY_SOURCE_YEARS.join(",");
  ...
  const [saved, profileCounts, bankCounts] = await Promise.all([
    ...
    getPool().query(`SELECT c.source_profile,COUNT(DISTINCT a.question_id)::int AS question_count,
      ...
      WHERE c.exam_track=ANY($1::text[]) AND c.status='approved' AND q.status='approved'
        ...
        AND ${contentCollectionSourceYearSql("c")} IN (${CONTENT_DELIVERY_YEAR_SQL})
  ...
  return contentRegistryReadSnapshots.read(snapshotKey, async () => {
    ...
    const result = await queryContentRegistryRead(`
      ...
      WHERE c.exam_track=ANY($1::text[])
        ...
        AND ${sourceYear} IN (${CONTENT_DELIVERY_YEAR_SQL})
...
function qbankQuestionProjection(
  ...
  { enforceDeliveryPolicy = false, collectionIdsParameter = "NULL" } = {},
) {
  const sourceYear = contentCollectionSourceYearSql("c");
  ...
  JOIN LATERAL (
    ...
    WHERE a.question_id=q.id AND c.status='approved' AND d.destination=$2 AND d.enabled=TRUE
      ...
      ${deliveryYearFilter}
    ORDER BY CASE WHEN ${sourceYear} IN (${CONTENT_DELIVERY_YEAR_SQL}) THEN 0 ELSE 1 END,
...
export async function getContentQbankQuestions({
  ...
  const cleanSourceProfile = normalizeContentSourceProfile(sourceProfile, '', { allowEmpty: true });
  const safeCollectionIds = normalizeContentCollectionIds(collectionIds);
  const result = await getPool().query(`${qbankQuestionProjection('$4', '$5', { collectionIdsParameter: '$6' })}
    WHERE q.exam_track=ANY($1::text[]) AND q.status='approved'
      AND COALESCE(NULLIF(q.source_data->>'item_format',''),'single_best_answer')='single_best_answer'
      AND ${contentQuestionDeliveryMediaReadySql("q")}
      AND q.id=ANY($3::uuid[])
