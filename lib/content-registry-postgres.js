...
function qbankQuestionProjection(
  ...
  SELECT q.id, q.student_qid, q.exam_track, q.title, q.question_html, q.explanation_html, q.correct_answer_id,
    ...
    COALESCE(q.taxonomy->>'topic_key',NULLIF(q.title,''),'unclassified') AS topic_key,
    COALESCE(q.taxonomy->>'subtopic_key','') AS subtopic_key,
    delivery.source_alias_id, delivery.source_item_id, delivery.source_provider, delivery.source_profile, delivery.display_policy,
  ...
  FROM content_questions q
  JOIN LATERAL (
    SELECT a.id AS source_alias_id, a.source_item_id, c.source_provider, c.source_profile, c.display_policy,
