...
export async function stageContentMediaImportAsset({ mediaImportJobId, entryIndex, asset }) {
  ...
  return contentMediaImportAsset(result.rows[0]);
}
...
export async function getContentMediaReferences(contentImportJobId, kind = "all") {
  ...
  const result = await getPool().query(`SELECT DISTINCT q.id AS question_id, q.student_qid, ref.value AS media_ref,
      COALESCE(q.source_data->'media_placements'->>ref.value, 'explanation') AS placement
  ...
  const rows = result.rows.map((row) => ({ questionId: row.question_id, studentQid: row.student_qid, mediaRef: row.media_ref, placement: row.placement }));
...
export async function listContentOperationalJobs({ limit = 50 } = {}) {
  ...
  const [questionJobs, mediaJobs, videoJobs] = await Promise.all([
    getPool().query(`SELECT id, exam_track, source_namespace, source_provider, collection_title,
      original_filename, zip_sha256, status, destinations, counts, errors, created_by, created_at, updated_at
  ...
  try {
    ...
    for (const asset of assets) {
      ...
      const result = await client.query(`INSERT INTO content_media_assets
        ...
        RETURNING id, object_key`, [id, parentJob.exam_track, parentJob.source_namespace, asset.sha256, asset.objectKey,
          asset.originalName, asset.contentType, asset.sizeBytes, asset.mediaKind || 'image', JSON.stringify({ media_import_job_id: mediaJobId, content_import_job_id: parentJob.id })]);
    ...
    for (const match of matches) {
      ...
      const result = await client.query(`INSERT INTO content_question_media
        ...
        ON CONFLICT (question_id, media_ref) DO NOTHING`, [crypto.randomUUID(), match.questionId, resolved.id, match.mediaRef, match.placement || 'explanation',
...
export async function listContentHubVideos({
  ...
  const result = await getPool().query(`WITH eligible AS (
      ...
      WHERE q.exam_track=ANY($1::text[])
        ...
        AND ($4='' OR COALESCE(NULLIF(q.taxonomy->>'topic_key',''),NULLIF(q.title,''),'Core review')=$4)
    ), grouped AS (
    ...
    SELECT 'registry-video:' || id::text AS resource_id,id AS video_asset_id,exam_track,
      ...
      primary_system_key AS system_key,primary_topic_key AS topic_key,
      ...
      delivery_destinations,systems,topics,
    ...
    FROM grouped
    ORDER BY primary_system_key,primary_topic_key,title,id
    LIMIT $5 OFFSET $6`, [examTracks, cleanDestinations, String(systemKey || ''), String(topicKey || ''), safeLimit, safeOffset]);
  return result.rows;
}

export async function getContentQbankCatalog({ examTrack, destination = 'aylamed_qbank', destinationScope = '' }) {
  await ensureContentRegistrySchema();
  ...
  `, [examTracks, destination, String(destinationScope || '')]);
  return result.rows;
}
...
function qbankQuestionProjection(destinationScopeParameter) { return `
...
export async function listContentQbankQuestions({
  ...
  const examTracks = qbankExamTrackAliases(examTrack);
  const result = await getPool().query(`${qbankQuestionProjection('$9')}
    WHERE q.exam_track=ANY($1::text[]) AND q.status='approved'
    ...
    LIMIT $8`, [examTracks, destination, String(systemKey), String(subsystemKey), String(topicKey), String(subtopicKey), safeSeed, safeLimit, String(destinationScope || '')]);
  return result.rows.map(contentQbankQuestionDisplay);
}

export async function getContentQbankQuestions({ questionIds = [], examTrack, destination = 'aylamed_qbank', destinationScope = '' } = {}) {
  await ensureContentRegistrySchema();
  const ids = [...new Set((Array.isArray(questionIds) ? questionIds : []).map(String).filter(Boolean))].slice(0, 200);
  if (!ids.length) return [];
  const examTracks = qbankExamTrackAliases(examTrack);
  const result = await getPool().query(`${qbankQuestionProjection('$4')}
    WHERE q.exam_track=ANY($1::text[]) AND q.status='approved' AND q.id=ANY($3::uuid[])
    ORDER BY array_position($3::uuid[],q.id)`, [examTracks, destination, ids, String(destinationScope || '')]);
  return result.rows.map(contentQbankQuestionDisplay);
}
