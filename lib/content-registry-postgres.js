...
export function contentQbankSelectionPredicate({ filters = {}, history, startIndex = 1, taxonomyExpression = 'q.taxonomy' } = {}) {
  ...
  return { values, sql: `
    ...
    AND (${p(5)}::jsonb IS NULL OR EXISTS (
      SELECT 1 FROM jsonb_array_elements(${p(5)}::jsonb) selected_path
      WHERE ${expressions.map((expression,index)=>`(NOT selected_path ? '${keys[index]}' OR ${expression}=selected_path->>'${keys[index]}')`).join('\n        AND ')}
    ))
    AND (${p(6)}='all'
