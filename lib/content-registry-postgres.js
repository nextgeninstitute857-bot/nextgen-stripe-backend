...
export async function updateContentCollectionControls({
  ...
  try {
    ...
    if (ownedOnly && normalizeContentSourceProfile(collection.source_profile, collection.source_provider) !== 'aylamed_original') {
      throw Object.assign(
        new Error('This AylaMed admin session can change controls only for AylaMed-owned original collections'),
