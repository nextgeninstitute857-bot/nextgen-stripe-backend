...
async function queryUworldCleanupSnapshot(client) {
  ...
  const { rows: [primary] } = await client.query(`
    ...
    SELECT
      ...
      (SELECT COUNT(*)::int FROM content_source_aliases a JOIN target_questions q ON q.question_id=a.question_id
        WHERE COALESCE(a.source_data->>'import_job_id','')<>$1) shared_aliases,
  ...
  const { rows: [media] } = await client.query(`
    WITH preview_questions AS (
      ...
      SELECT DISTINCT object_key FROM content_media_import_assets WHERE media_import_job_id=$1
      UNION
    ...
    SELECT
      ...
      (SELECT COALESCE(SUM(size_bytes),0)::text FROM assets) registered_bytes,
      (SELECT COUNT(*)::int FROM content_question_media qm JOIN assets a ON a.id=qm.media_asset_id
       WHERE qm.question_id NOT IN (
         SELECT question_id FROM content_source_aliases WHERE source_data->>'import_job_id'=$2
         UNION SELECT question_id FROM preview_questions
       )) shared_media_links
  ...
  const checks = {
    ...
    no_shared_aliases: Number(primary?.shared_aliases) === 0,
    ...
    no_shared_media: Number(media?.shared_media_links) === 0,
  };
...
export async function applyGuardedUworldCleanup({ expectedFingerprint = "" } = {}) {
  ...
  try {
    ...
    await client.query("LOCK TABLE content_import_jobs, content_source_aliases, content_collections, content_questions, content_media_assets IN SHARE ROW EXCLUSIVE MODE");
    ...
    await client.query(`CREATE TEMP TABLE uworld_cleanup_questions ON COMMIT DROP AS
    ...
    const deletedQuestions = await client.query(`DELETE FROM content_questions q
      WHERE q.id IN (SELECT question_id FROM uworld_cleanup_questions)
        AND NOT EXISTS (SELECT 1 FROM content_source_aliases a WHERE a.question_id=q.id)
      RETURNING q.id`);
    await client.query(`DELETE FROM content_media_assets ma WHERE ma.object_key=ANY($1::text[])
      AND NOT EXISTS (SELECT 1 FROM content_question_media qm WHERE qm.media_asset_id=ma.id)`, [objectKeys]);
