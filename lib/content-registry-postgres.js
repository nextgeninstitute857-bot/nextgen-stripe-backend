  ...
  const result = await getPool().query(`
    SELECT c.*,
      ...
      COUNT(DISTINCT q.id) FILTER (WHERE COALESCE(q.taxonomy->>'system_key','')<>''
        ...
        AND COALESCE(q.taxonomy->>'topic_key','')<>''
        AND COALESCE(q.taxonomy->>'subtopic_key','')<>'')::int AS taxonomy_complete_count,
      COUNT(DISTINCT q.id) FILTER (WHERE q.source_data->>'source_adapter'='aylamed_owned_sba_v1')::int
        AS native_question_count,
      COUNT(DISTINCT q.id) FILTER (WHERE q.source_data->>'source_adapter'='aylamed_owned_sba_v1'
        AND COALESCE((q.source_data->>'private_import_ready')::boolean,FALSE))::int AS private_ready_count,
      COUNT(DISTINCT q.id) FILTER (WHERE q.source_data->>'source_adapter'='aylamed_owned_sba_v1'
        AND COALESCE((q.source_data->>'publication_gate_ready')::boolean,FALSE))::int AS publication_ready_count,
      COUNT(DISTINCT q.id) FILTER (WHERE q.source_data->>'source_adapter'='aylamed_owned_sba_v1'
        AND ${contentQuestionMediaReadySql("q")})::int AS media_ready_count,
      COUNT(DISTINCT q.id) FILTER (WHERE ${contentQuestionMediaReadySql("q")})::int
    ...
    GROUP BY c.id,recovered_job.draft_import_job_id,recovered_job.draft_import_job_status
    ORDER BY c.created_at DESC LIMIT $${values.length - 1} OFFSET $${values.length}
  `, values);
  return result.rows;
}

export async function getContentGlobalQbankPublicationState({ examTrack = 'usmle-step-1' } = {}) {
  await ensureContentRegistrySchema();
  const result = await getPool().query(`
