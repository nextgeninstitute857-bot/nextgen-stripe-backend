...
function contentDeliveryFallbackTaxonomyReadySql(questionAlias = "q", collectionAlias = "c", _examTrack = "") {
  const sourceYear = contentCollectionSourceYearSql(collectionAlias);
  return `(
    ${sourceYear}=2026
    OR (
      NULLIF(${questionAlias}.taxonomy->>'system_key','') IS NOT NULL
      AND NULLIF(${questionAlias}.taxonomy->>'topic_key','') IS NOT NULL
    )
  )`;
}
