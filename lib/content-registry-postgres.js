...
function buildStep1CollectionTaxonomyRepairPreview(collection, questions, destinationControls) {
  ...
  if (activeDeliveries || approvedQuestions) blockers.push("approved_or_delivered_question_present");
  ...
  try {
    ...
    if (built.repairRows.length) {
      const payload = built.repairRows.map((row) => ({
        ...
        repair: {
          operation: "step1_source_taxonomy_in_place_v1",
          collection_id: String(collectionId),
          ledger_schema_version: row.taxonomy.ledger_schema_version,
          ledger_fingerprint: row.taxonomy.ledger_fingerprint,
          applied_by: String(actorId || ""),
        },
      }));
      await client.query(`UPDATE content_questions q SET
          taxonomy=x.taxonomy,
          source_data=COALESCE(q.source_data,'{}'::jsonb) || jsonb_build_object(
            'source_adapter',$2,
            'taxonomy_import_ready',TRUE,
            'taxonomy_publication_ready',FALSE,
            'source_taxonomy_ledger',jsonb_build_object(
              'schema_version',x.taxonomy->>'ledger_schema_version',
              'fingerprint',x.taxonomy->>'ledger_fingerprint',
              'native_sys_id',x.taxonomy->'native_sys_id',
              'native_sub_id',x.taxonomy->'native_sub_id'
            ),
            'taxonomy_repair',x.repair
          ),
          updated_at=NOW()
        FROM jsonb_to_recordset($1::jsonb) AS x(question_id text,taxonomy jsonb,repair jsonb)
