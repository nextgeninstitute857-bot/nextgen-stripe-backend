...
export function contentQbankSelectionPredicate({ filters = {}, history, startIndex = 1 } = {}) {
  ...
  const expressions = [
    `COALESCE(NULLIF(q.taxonomy->>'system_key',''),NULLIF(q.system_key,''),'unclassified')`,
    `COALESCE(q.taxonomy->>'subsystem_key','')`,
    `COALESCE(q.taxonomy->>'topic_key',NULLIF(q.title,''),'unclassified')`,
    `COALESCE(q.taxonomy->>'subtopic_key','')`,
  ];
...
export function contentQbankFacetQuery({ examTrack, destination = 'aylamed_qbank', destinationScope = '', sourceProfile = '', collectionIds = null, filters = {}, history } = {}) {
  const selection = contentQbankSelectionPredicate({ filters, history, startIndex: 6 });
  const sourceYear = contentCollectionSourceYearSql('c');
  ...
  sql: `FROM content_questions q
    JOIN LATERAL (
      SELECT a.id AS source_alias_id
      FROM content_source_aliases a
      JOIN content_collection_destinations d ON d.collection_id=a.collection_id
      JOIN content_collections c ON c.id=a.collection_id
...
export function contentQbankFacetsQuery(options = {}) {
  ...
  return { values: query.values, sql: `WITH eligible AS (
    SELECT q.id, q.taxonomy,
      COALESCE(NULLIF(q.taxonomy->>'system_key',''),NULLIF(q.system_key,''),'unclassified') AS system_key,
      COALESCE(q.taxonomy->>'source','') AS taxonomy_source,
      (q.taxonomy->>'source' IN ('question_override','aylamed_owned_json') OR
        (q.taxonomy->>'source'='provider_mapping' AND q.taxonomy->>'review_status'='approved')) IS TRUE AS reviewed
    ${query.sql}
  ), visible AS (
...
export async function getContentQbankCatalog({
  ...
  const result = await getPool().query(`
    ...
    FROM content_questions q
    JOIN LATERAL (
      SELECT a.id AS source_alias_id, ${sourceYear} AS source_year
      FROM content_source_aliases a
      JOIN content_collection_destinations d ON d.collection_id=a.collection_id
      JOIN content_collections c ON c.id=a.collection_id
      WHERE a.question_id=q.id AND c.status='approved' AND d.destination=$2 AND d.enabled=TRUE
  ...
  `, [examTracks, destination, String(destinationScope || ''), cleanSourceProfile, safeCollectionIds]);
  return result.rows;
}
