...
function contentDeliveryYearSql(examTrack = "") {
  return contentDeliverySourceYearsForExam(examTrack).join(",");
}
...
function contentDeliveryFallbackTaxonomyReadySql(questionAlias = "q", collectionAlias = "c", examTrack = "") {
  if (contentDeliverySourceYearsForExam(examTrack).length <= 1) return "TRUE";
  ...
  const [saved, profileCounts, bankCounts] = await Promise.all([
    ...
    getPool().query(`SELECT c.source_profile,COUNT(DISTINCT a.question_id)::int AS question_count,
      ...
      WHERE c.exam_track=ANY($1::text[]) AND c.status='approved' AND q.status='approved'
        ...
        AND ${contentCollectionSourceYearSql("c")} IN (${contentDeliveryYearSql(examTrack)})
  ...
  return contentRegistryReadSnapshots.read(snapshotKey, async () => {
    ...
    const result = await queryContentRegistryRead(`
      ...
      WHERE c.exam_track=ANY($1::text[])
        ...
        AND ${sourceYear} IN (${contentDeliveryYearSql(examTrack)})
        AND ${contentDeliveryFallbackTaxonomyReadySql("q", "c", examTrack)}
...
function qbankQuestionProjection(
  ...
  JOIN LATERAL (
    ...
    WHERE a.question_id=q.id AND c.status='approved' AND d.destination=$2 AND d.enabled=TRUE
      ...
      ${deliveryYearFilter}
      AND ${contentDeliveryFallbackTaxonomyReadySql("q", "c", examTrack)}
    ORDER BY CASE WHEN ${sourceYear} IN (${contentDeliveryYearSql(examTrack)}) THEN 0 ELSE 1 END,
