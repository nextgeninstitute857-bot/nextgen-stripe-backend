...
export async function createContentImportJob(job) {
  ...
  await getPool().query(`INSERT INTO content_import_jobs
    (id, exam_track, source_namespace, source_provider, source_profile, collection_title, original_filename, zip_sha256, destinations, created_by)
    ...
    JSON.stringify(job.destinations || []), job.createdBy,
  ]);
}
...
export async function importContentQuestionBatch({ job, collectionKey, collectionTitle, rows, destinations = [] }) {
  ...
  try {
    ...
    const collectionResult = await client.query(`INSERT INTO content_collections
      (id, exam_track, source_namespace, source_provider, source_profile, collection_key, title, status, destinations)
      ...
      ON CONFLICT (exam_track, source_namespace, collection_key) DO UPDATE SET
        ...
        source_profile=EXCLUDED.source_profile, destinations=EXCLUDED.destinations, updated_at=NOW()
      RETURNING id`, [
...
export async function updateContentCollectionControls({
  ...
  try {
    ...
    await client.query(`UPDATE content_collections SET
      ...
      approved_by=CASE WHEN $2='approved' THEN $5 WHEN $2='draft' THEN NULL ELSE approved_by END,
