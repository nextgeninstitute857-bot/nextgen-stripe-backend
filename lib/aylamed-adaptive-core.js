...
export function buildAylaMistakeFlashcard({
  ...
  const front = cleanString(question.question_html ?? question.questionHtml ?? question.stem ?? question.question ?? question.title, 12000);
  const back = correctAnswerText(question, correctAnswer);
  ...
  const explanation = cleanString(question.explanation_html ?? question.explanationHtml ?? question.explanation, 12000);
