...
export const AYLA_VIMEO_MAPPING_EXPECTED_COUNT = 491;
export const AYLA_VIMEO_MAPPING_FOLDER_ID = "29973623";
...
export function validateAylaVimeoMappingImport({ mappings = [], drafts = [] } = {}) {
  ...
  const catalogue = Array.isArray(drafts) ? drafts : Object.values(drafts || {});
  const catalogueByVimeoId = new Map(catalogue.map((row) => [text(row.vimeoId ?? row.vimeo_id, 80), row]));
  const seen = new Set();
  const errors = [];
  const warnings = [];

  if (rows.length !== AYLA_VIMEO_MAPPING_EXPECTED_COUNT) {
    errors.push(`Expected exactly ${AYLA_VIMEO_MAPPING_EXPECTED_COUNT} mappings; received ${rows.length}`);
  }
  if (catalogue.length !== AYLA_VIMEO_MAPPING_EXPECTED_COUNT) {
    errors.push(`Expected exactly ${AYLA_VIMEO_MAPPING_EXPECTED_COUNT} catalogue drafts; found ${catalogue.length}`);
  }
  ...
  rows.forEach((row, index) => {
    ...
    if (draft && text(draft.folderId ?? draft.folder_id, 80) !== AYLA_VIMEO_MAPPING_FOLDER_ID) {
      errors.push(`${label}: catalogue draft is outside Vimeo folder ${AYLA_VIMEO_MAPPING_FOLDER_ID}`);
    }
  ...
  return {
    ...
    dry_run: true,
    expected_count: AYLA_VIMEO_MAPPING_EXPECTED_COUNT,
    ...
    folder_id: AYLA_VIMEO_MAPPING_FOLDER_ID,
    errors,
    warnings,
