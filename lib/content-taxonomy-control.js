...
export function normalizeContentTaxonomy(taxonomy = {}, { requireTopic = false } = {}) {
  ...
  if (requireTopic && !normalized.topic_key) throw Object.assign(new Error("topic_key is required for a question override"), { statusCode: 400 });
  return normalized;
}
...
export function contentTaxonomyMappingReviewState(mapping = {}) {
  ...
  return status === "active" && reviewStatus === "approved" ? "approved" : "needs_review";
}
...
function mergeMetric(target, metric = {}) {
  const countFields = [
    ...
    "topic_classified_questions", "complete_questions", "question_override_count",
