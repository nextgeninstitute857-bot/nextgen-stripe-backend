...
function actionValue(actions, acceptedTypes) {
  ...
  return (Array.isArray(actions) ? actions : [])
    .filter((item) => types.has(clean(item?.action_type)))
...
export function metaAdsReadiness(env = process.env) {
  ...
  const preferredAccount = clean(env.META_AD_ACCOUNT_ID);
  ...
  return {
    ...
    preferred_account_configured: Boolean(preferredAccount),
...
async function graphGetAll({ axiosClient, url, token, params = {}, maxPages = 10 }) {
  ...
  while (next && page < maxPages) {
    ...
    output.push(...(Array.isArray(response?.data?.data) ? response.data.data : []));
...
export async function fetchMetaAdsSnapshot({ axiosClient, env = process.env, datePreset = "last_30d" }) {
  ...
  const token = clean(env.META_ADS_ACCESS_TOKEN);
  const preferredAccount = cleanId(env.META_AD_ACCOUNT_ID);
  ...
  const accounts = accountRows
    ...
    .filter((account) => !preferredAccount || account.meta_account_id === preferredAccount);

  if (preferredAccount && !accounts.length) {
    const error = new Error("The configured Meta ad account is not available to this access token");
  ...
  for (const account of accounts) {
    ...
    const [campaignRows, adSetRows, adRows, insightRows] = await Promise.all([
      graphGetAll({
        axiosClient,
