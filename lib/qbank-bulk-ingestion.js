...
const AUTHORIZED_EXTERNAL_QBANK_RELEASE_SCOPES = Object.freeze({
  amedex: { exam_track: "amc", source_profile: "amedex_style" },
  mplusx: { exam_track: "amc", source_profile: "mplusx_style" },
  bmjonexamination: { exam_track: "plab", source_profile: "other" },
  canadaqbank: { exam_tracks: ["mccqe", "plab"], source_profile: "canadaqbank_style" },
  aceqbank: { exam_track: "mccqe", source_profile: "aceqbank_style" },
  passmedicine: { exam_track: "plab", source_profile: "other" },
  boardvitals: { exam_track: "nclex", source_profile: "other" },
  uworld: { exam_tracks: ["usmle-step-2", "usmle-step-3", "nclex"], source_profile: "uworld_style" },
  amboss: { exam_tracks: ["usmle-step-1", "usmle-step-2", "usmle-step-3"], source_profile: "amboss_style" },
});
...
export function contentAuthorizedExternalReleaseAllowed(input = {}) {
  const rights = normalizeContentRightsStatus(
    input.source_rights_status || input.sourceRightsStatus || "unverified",
  );
  if (!new Set(["licensed", "authorized"]).has(rights)) return false;
  const provider = cleanString(
    input.source_provider || input.sourceProvider,
    160,
  ).toLowerCase().replace(/[^a-z0-9]+/g, "");
  const scope = AUTHORIZED_EXTERNAL_QBANK_RELEASE_SCOPES[provider];
  if (!scope) return false;
  const examTrack = normalizeExamTrack(input.exam_track || input.examTrack);
  const examAllowed = scope.exam_tracks
    ? scope.exam_tracks.includes(examTrack)
    : scope.exam_track === examTrack;
  if (!examAllowed) return false;
  return normalizeBulkQbankSourceProfile(
