export const MULTI_QBANK_INGESTION_BUILD = "v249-question-zip-directory-cache";
...
export function multiQbankIngestionConfig(source = process.env) {
  ...
  return {
    ...
    lane_concurrency: {
      ...
      // A classifier keeps the Ayla database cache, Vimeo evidence, taxonomy
      // candidates, and an OpenAI response in the same V8 heap. Keep this lane
      // single-filed until both the process heap and the persistence model are
      // deliberately resized and load-tested together.
      ayla_vimeo_ai: 1,
      // A single durable run classifies provider pairs sequentially. Keeping
      // this lane single-filed prevents overlapping catalog-wide passes while
      // still allowing ordinary imports and the Vimeo lane to make progress.
      ayla_taxonomy_ai: 1,
    },
