...
import { normalizeAylaQbankSelectionPaths, normalizeAylaQbankUsageStatus } from './aylamed-qbank-facets.js';
import { projectStudentQbankStem, projectStudentQbankHintMedia } from "./aylamed-qbank-student-html.js";

const QBANK_MODES = new Set(["tutor", "test"]);
const QBANK_PURPOSES = new Set(["practice", "baseline_diagnostic"]);
const QBANK_DIFFICULTIES = new Set(["adaptive", "easy", "medium", "hard"]);
...
export function normalizeAylaQbankDifficulty(value = "adaptive") {
  const clean = String(value || "adaptive").trim().toLowerCase().replace(/[\s_-]+/g, "-");
  if (QBANK_DIFFICULTIES.has(clean)) return clean;
  if (["all", "any", "mixed", "all-levels"].includes(clean)) return "adaptive";
  throw qbankError("QBank difficulty must be adaptive, easy, medium, or hard", 400, "INVALID_QBANK_DIFFICULTY");
}

export function normalizeAylaQbankFilters(input = {}) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw qbankError('QBank filters must be an object');
  for (const field of ['system_key', 'systemKey', 'subsystem_key', 'subsystemKey', 'topic_key', 'topicKey', 'subtopic_key', 'subtopicKey']) {
    if (input[field] !== undefined && input[field] !== null && typeof input[field] !== 'string') throw qbankError('Legacy taxonomy filters must be strings; use selection_paths for multiple branches');
  }
  const filters = {
  ...
  if (input.difficulty !== undefined && input.difficulty !== null && String(input.difficulty).trim()) {
    filters.difficulty = normalizeAylaQbankDifficulty(input.difficulty);
  }
  if (input.selection_paths !== undefined) filters.selection_paths = normalizeAylaQbankSelectionPaths(input.selection_paths);
  if (input.status !== undefined) filters.status = normalizeAylaQbankUsageStatus(input.status);
  return filters;
}
...
export function createAylaQbankSession({
  ...
  for (const row of Array.isArray(questions) ? questions : []) {
    ...
    cleanQuestions.push({
      ref,
      contentQuestionId,
      collectionId: cleanString(row?.collectionId || row?.collection_id, 100) || null,
      collectionKey: cleanString(row?.collectionKey || row?.collection_key, 160) || null,
      bankName: cleanString(row?.bankName || row?.bank_name, 240) || null,
      sourceExamTrack: normalizeAylaQbankExamTrack(row?.sourceExamTrack || row?.source_exam_track) || cleanExamTrack,
      supplemental: row?.supplemental === true,
      scoringAllowed: row?.scoringAllowed !== false && row?.scoring_allowed !== false,
    });
  }
  ...
  return {
    ...
    sourceProfile: sourceProfile ? cleanString(sourceProfile, 80).toLowerCase() : null,
    selectedBanks: (Array.isArray(selectedBanks) ? selectedBanks : []).map((row) => ({
      collectionId: cleanString(row?.collectionId || row?.collection_id || row?.id, 100),
      collectionKey: cleanString(row?.collectionKey || row?.collection_key, 160),
      name: cleanString(row?.name || row?.bankName || row?.bank_name, 240),
      sourceExamTrack: normalizeAylaQbankExamTrack(row?.sourceExamTrack || row?.source_exam_track) || cleanExamTrack,
      supplemental: row?.supplemental === true,
      scoringAllowed: row?.scoringAllowed !== false && row?.scoring_allowed !== false,
    })).filter((row) => row.collectionId),
    destinationScope: destinationScope ? cleanString(destinationScope, 120).toLowerCase() : null,
    status: "in_progress",
...
export function recordAylaQbankAnswer(session = {}, {
  ...
  const answer = {
    ...
    collectionKey: question.collectionKey || null,
    bankName: question.bankName || null,
    sourceExamTrack: question.sourceExamTrack || session.examTrack,
    supplemental: question.supplemental === true,
    scoringAllowed: question.scoringAllowed !== false,
...
function qbankSessionScoreSummary(session = {}) {
  const questions = Array.isArray(session.questions) ? session.questions : [];
  const total = Math.max(0, Number(session.questionCount || questions.length || 0));
  // Compact legacy history may have stored counters without question mappings.
  // Preserve its score while separating its recorded unanswered count.
  if (!questions.length) {
    const storedSupplementalCount = Math.max(0, Number(session.supplementalQuestionCount || 0));
    const scoredQuestionCount = Math.max(0, Number(session.scoredQuestionCount ?? total - storedSupplementalCount));
    const correctCount = Math.min(scoredQuestionCount, Math.max(0, Number(session.correctCount || 0)));
    const unansweredCount = Math.min(scoredQuestionCount - correctCount, Math.max(0, Number(session.unansweredCount || 0)));
    return {
      answeredCount: Number(session.answeredCount || 0),
      scoredAnsweredCount: Math.max(0, scoredQuestionCount - unansweredCount),
      scoredQuestionCount,
      supplementalQuestionCount: Math.max(0, Number(session.supplementalQuestionCount ?? total - scoredQuestionCount)),
      correctCount,
      incorrectCount: Math.max(0, scoredQuestionCount - correctCount - unansweredCount),
      unansweredCount,
      scorePercent: scoredQuestionCount ? Number(session.scorePercent || 0) : null,
    };
  }
  const answers = Object.entries(session.answers || {});
  const scoredRefs = new Set(questions
    .filter((row) => row.scoringAllowed !== false)
    .map((row) => String(row.ref || ""))
    .filter(Boolean));
  // The answer map key is the original durable reference in older sessions.
  const scoredAnswers = answers
    .filter(([ref, row]) => scoredRefs.has(String(row.questionRef || ref)))
    .map(([, row]) => row);
  const scoredQuestionCount = scoredRefs.size;
  const supplementalQuestionCount = Math.max(0, total - scoredQuestionCount);
  const correctCount = scoredAnswers.filter((row) => row.correct === true).length;
  const answeredCount = answers.length;
  const scoredAnsweredCount = scoredAnswers.length;
  const scorePercent = scoredQuestionCount
    ? Math.round((correctCount / scoredQuestionCount) * 10000) / 100
    : null;
  return {
    answeredCount,
    scoredAnsweredCount,
    ...
    correctCount,
    incorrectCount: Math.max(0, scoredAnsweredCount - correctCount),
    unansweredCount: Math.max(0, scoredQuestionCount - scoredAnsweredCount),
    scorePercent,
  };
}
...
export function finalizeAylaQbankSession(session = {}, now = new Date()) {
  ...
  const summary = qbankSessionScoreSummary(session);
...
export function sanitizeAylaQbankQuestion(question = {}, { session = {}, questionRef = "", bookmark = null, note = null, revision = null } = {}) {
  ...
  const stem = projectStudentQbankStem(question.question_html);
  const projected = projectStudentQbankHintMedia(question, stem, reveal);
  const answer = session.answers?.[String(questionRef)] || null;
  const draft = session.mode === "test" && session.status === "in_progress"
    ? session.draftAnswers?.[String(questionRef)] || null : null;
  ...
  const visibleMedia = (Array.isArray(projected.media) ? projected.media : [])
  ...
  for (const item of visibleMedia) {
    const match = String(item.placement || "").match(/^answer:(\d+)$/);
    if (!match) continue;
    const answerId = Number(match[1]);
    if (!answerMedia.has(answerId)) answerMedia.set(answerId, []);
    answerMedia.get(answerId).push(item);
  }
  const media = visibleMedia.filter((item) => !/^answer:\d+$/.test(String(item.placement || "")));
  const videos = (Array.isArray(projected.videos) ? projected.videos : [])
    .filter((item) => reveal || String(item.placement || "explanation") !== "explanation")
    .map((item) => ({
      id: item.id,
      ref: item.ref || null,
  ...
  const choices = (Array.isArray(question.answers) ? question.answers : []).map((row) => ({
    ...
    media: answerMedia.get(Number(row.answer_id ?? row.answerId)) || [],
  }));
  const result = {
    ...
    answered: Boolean(answer || draft),
    draft_saved: Boolean(draft && !answer),
    selected_answer_id: answer || draft ? Number((answer || draft).selectedAnswerId) : null,
...
export function sanitizeAylaQbankSession(session = {}, { includeInternal = false } = {}) {
  // Derive presentation from durable answer/mapping evidence, so legacy result
  // counters are corrected on read without rewriting scores or finalizing again.
  const summary = qbankSessionScoreSummary(session);
  const questions = (Array.isArray(session.questions) ? session.questions : []).map((row) => ({
    question_ref: row.ref,
    ...(row.collectionId ? { bank: {
      collection_id: row.collectionId,
      collection_key: row.collectionKey || null,
      name: row.bankName || row.collectionKey || "Question Bank",
      source_exam_track: row.sourceExamTrack || session.examTrack,
      supplemental: row.supplemental === true,
      scoring_allowed: row.scoringAllowed !== false,
    } } : {}),
    ...(includeInternal ? { content_question_id: row.contentQuestionId } : {}),
  }));
  return {
    id: session.id,
    ...
    source_profile: session.sourceProfile || null,
    selected_banks: (Array.isArray(session.selectedBanks) ? session.selectedBanks : []).map((row) => ({
      collection_id: row.collectionId,
      collection_key: row.collectionKey || null,
      name: row.name || row.collectionKey || "Question Bank",
      source_exam_track: row.sourceExamTrack || session.examTrack,
      supplemental: row.supplemental === true,
      scoring_allowed: row.scoringAllowed !== false,
    })),
    ...
    content_delivery_policy: session.contentDeliveryPolicy || null,
    content_delivery_selection: session.contentDeliverySelection || null,
    diagnostic_coverage: session.purpose === "baseline_diagnostic" && session.diagnosticCoverage
      ...
      : null,
    answered_count: session.status === "in_progress" && session.mode === "test"
      ? new Set([...Object.keys(session.answers || {}), ...Object.keys(session.draftAnswers || {})]).size
      : summary.answeredCount,
    draft_version: Number(session.draftVersion || 0),
    draft_count: Object.keys(session.draftAnswers || {}).length,
    adaptation: session.adaptation ? {
      status: session.adaptation.status,
      date: session.adaptation.date,
      updated_at: session.adaptation.updatedAt || null,
      plan_id: session.adaptation.planId || null,
    } : null,
    scored_answered_count: String(session.status) === "submitted" ? summary.scoredAnsweredCount : null,
    scored_question_count: summary.scoredQuestionCount,
    supplemental_question_count: summary.supplementalQuestionCount,
    correct_count: String(session.status) === "submitted" ? summary.correctCount : null,
    incorrect_count: String(session.status) === "submitted" ? summary.incorrectCount : null,
    unanswered_count: String(session.status) === "submitted" ? summary.unansweredCount : null,
    score_percent: String(session.status) === "submitted" ? summary.scorePercent : null,
...
export function qbankSessionHistoryRow(session = {}) {
  ...
  return {
    ...
    selected_banks: safe.selected_banks,
    question_count: safe.question_count,
    answered_count: safe.answered_count,
    scored_question_count: safe.scored_question_count,
    supplemental_question_count: safe.supplemental_question_count,
