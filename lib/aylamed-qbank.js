...
export function finalizeAylaQbankSession(session = {}, now = new Date()) {
  ...
  const scoredRefs = new Set(questions
    ...
    .filter(Boolean));
  const scoredAnswers = answers.filter((row) => scoredRefs.has(String(row.questionRef || "")));
  ...
  return {
    ...
    session: {
      ...
      correctCount,
      incorrectCount: Math.max(0, scoredQuestionCount - correctCount),
...
export function sanitizeAylaQbankSession(session = {}, { includeInternal = false } = {}) {
  ...
  return {
    ...
    answered_count: session.status === "in_progress" && session.mode === "test"
      ...
      : Number(session.answeredCount || 0),
    ...
    scored_answered_count: String(session.status) === "submitted" ? Number(session.scoredAnsweredCount || 0) : null,
    scored_question_count: Number(session.scoredQuestionCount ?? session.questionCount ?? questions.length),
    supplemental_question_count: Number(session.supplementalQuestionCount || 0),
