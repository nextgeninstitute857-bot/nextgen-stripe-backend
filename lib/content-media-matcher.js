...
function resolveCandidateSet(candidates, reference = {}, {
  ...
  if (typeof candidatePriority === "function") {
    const ranked = unique
      .map((asset) => ({
        asset,
        rank: Number(candidatePriority(asset, reference)),
      }))
      .filter((item) => Number.isFinite(item.rank) && item.rank >= 0)
      .sort((left, right) => left.rank - right.rank);
    if (ranked.length) {
      const preferred = ranked
        .filter((item) => item.rank === ranked[0].rank)
        .map((item) => item.asset);
      if (preferred.length === 1) {
        return { resolved: preferred[0], ambiguous: [] };
      }
      return { resolved: null, ambiguous: preferred };
    }
  }
  return {
    resolved: null,
