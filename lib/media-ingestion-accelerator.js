...
export function mediaAcceleratorConfig(source = process.env) {
  return {
    concurrency: boundedInteger(source.NEXTGEN_CONTENT_MEDIA_CONCURRENCY, 8, 1, 12),
...
export async function finalizeMediaInBatches({
  ...
  const safeBatchSize = boundedInteger(batchSize, 1_000, 25, 1_000);
...
export class BoundedTaskPool {
  constructor(limit = 8, { onError } = {}) {
    this.limit = boundedInteger(limit, 8, 1, 12);
