...
export function contentMediaStatus() {
  const configured = Boolean(env("CLOUDFLARE_R2_ACCOUNT_ID") && env("CLOUDFLARE_R2_ACCESS_KEY_ID") && env("CLOUDFLARE_R2_SECRET_ACCESS_KEY") && env("CLOUDFLARE_R2_BUCKET"));
  return { configured, provider: "cloudflare-r2", bucket: configured ? env("CLOUDFLARE_R2_BUCKET") : null, public_access: false };
}
