...
export async function uploadMediaZipToR2({
  ...
  const zip = await openZip(zipSource, { autoClose: false });
  const referenceKeys = mediaReferenceKeys(references);
  ...
  const resumedBytes = [...resumedByEntry.values()]
  ...
  await new Promise((resolve, reject) => {
    ...
    zip.on("entry", async (entry) => {
      try {
        ...
        const candidate = mediaEntryCandidate(entry, referenceKeys);
        if (!candidate) {
          if (entries % 500 === 0) await emitProgress(false);
        ...
        if (staged) {
          ...
          if (resumedFilesSeen % 500 === 0) await emitProgress(false);
          if (!settled) zip.readEntry();
          return;
        }
