...
const MAX_SAFE_SVG_BYTES = Math.max(64 * 1024, Number(process.env.NEXTGEN_CONTENT_MAX_SVG_BYTES || 5 * 1024 ** 2));
...
export function safeMediaEntryName(value, { allowSvg = false } = {}) {
  ...
  return IMAGE_EXTENSIONS.has(extension) || AUDIO_EXTENSIONS.has(extension) || (allowSvg && extension === ".svg") ? normalized : null;
}
...
export function matchMediaReferences(references = [], assets = [], options = {}) {
  return matchReferencedAssets(references, assets, {
    ...options,
    matchField: "asset",
  });
}

export function contentMediaReferenceKeys(references = []) {
  return new Set(references.flatMap((reference) => [
    reference.mediaRef,
    ...(Array.isArray(reference.matchPaths)
      ? reference.matchPaths
      : reference.matchPath
        ? [reference.matchPath]
        : []),
  ].flatMap((value) => mediaMatchKeys(value))));
}

function mediaEntryCandidate(entry, referenceKeys, { allowSvg = false } = {}) {
  const originalName = safeMediaEntryName(entry.fileName, { allowSvg });
  ...
  const maxAssetBytes = path.extname(originalName).toLowerCase() === ".svg"
    ? MAX_SAFE_SVG_BYTES
    : mediaKind === "audio" ? MAX_AUDIO_BYTES : MAX_IMAGE_BYTES;
...
export async function inspectMediaZip({
  ...
  const referenceKeys = contentMediaReferenceKeys(references);
  let entries = 0;
  ...
  await new Promise((resolve, reject) => {
    zip.on("entry", async (entry) => {
      try {
        ...
        const candidate = mediaEntryCandidate(entry, referenceKeys, { allowSvg });
        if (candidate) {
          candidateEntries += 1;
...
export async function uploadMediaZipToR2({
  ...
  const uploadEntry = async ({ entry, entryIndex, candidate }) => {
    ...
    try {
      if (typeof transferGate?.acquire === "function") {
        releaseTransfer = await transferGate.acquire({
          metadata: {
            media_import_job_id: String(mediaImportJobId || importJobId),
            entry_index: entryIndex,
          },
          onWait: (gate) => emitProgress(true, {
            transfer_capacity_waiting: true,
            global_transfer_gate: gate,
          }),
        });
      }
      transferStartedAtMs ??= Date.now();
      const { originalName, mediaKind, maxAssetBytes } = candidate;
      let input = await openEntry(zip, entry, {
        timeoutMs: Number(
          zip?.contentRecovery?.entry_open_timeout_ms
            || recoveryConfig.entry_open_timeout_ms,
        ),
      ...
      if (path.extname(originalName).toLowerCase() === ".svg") {
        ...
        for await (const chunk of input) {
          ...
          svgBytes += value.length;
          if (svgBytes > MAX_SAFE_SVG_BYTES) {
            throw Object.assign(new Error(`SVG exceeds limit: ${originalName}`), { statusCode: 413 });
          }
          chunks.push(value);
        }
        const svg = Buffer.concat(chunks).toString("utf8");
        if (!/<svg\b/i.test(svg)
          || /<!doctype|<!entity|<script\b|<foreignObject\b|\bon[a-z]+\s*=|\b(?:href|xlink:href)\s*=\s*["']\s*(?:https?:|\/\/|javascript:|data:)|url\(\s*["']?\s*(?:https?:|\/\/|javascript:|data:)/i.test(svg)) {
          throw Object.assign(new Error(`SVG contains unsafe active or external content: ${originalName}`), { statusCode: 400 });
        }
        input = Readable.from(Buffer.from(svg, "utf8"));
      }
      ...
      releaseTransfer?.({ error: transferError });
    }
  };
