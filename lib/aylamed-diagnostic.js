...
export const AYLA_DIAGNOSTIC_BLUEPRINT_VERSION = 5;
...
const TITLE_RULES = Object.freeze([
  ...
  },
  {
    ...
    pattern: /\b(?:behavio(?:u)?r\w*|psychiatr\w*|anxiety|depress\w*|deliri\w*|personality|sleep disorder)\b/i,
  },
  {
    system: "Pharmacology",
  ...
  {
    system: "Biochemistry",
    pattern: /\b(?:beta oxidation|mosaicism|lesch[- ]nyhan|genomic imprinting|genetic disorders?|embryologic derivatives|laboratory techniques|biochem\w*|genetic\w*|chromosom\w*|metaboli\w*|rna|protein synthesis|translation|transcription|apoptosis)\b/i,
  },
]);

// Imported Step 1 catalogues use a deliberately broader source taxonomy than
// the student diagnostic blueprint. For example, UWorld groups Neurology and
// Behavioral Science under one source system, but its verified subsystem label
// still identifies the correct diagnostic lane. Resolve those reviewed labels
// before falling back to title heuristics so an imported, fully mapped bank is
// not incorrectly treated as unclassified diagnostic content.
const SUBSYSTEM_RULES = Object.freeze([
  { system: "Cardiovascular", pattern: /\b(?:cardiovascular|cardiology)\b/i },
  { system: "Renal", pattern: /\b(?:renal|urinary|nephrology)\b/i },
  { system: "Respiratory", pattern: /\b(?:pulmonary|respiratory|critical care)\b/i },
  { system: "Gastrointestinal", pattern: /\b(?:gastrointestinal|gastroenterology|nutrition|hepatology)\b/i },
  { system: "Neurology", pattern: /\b(?:nervous system|neurology|neuroscience|ear nose throat|ophthalmology|special senses)\b/i },
  { system: "Endocrine", pattern: /\b(?:endocrine|diabetes|metabolism)\b/i },
  { system: "Reproductive", pattern: /\b(?:female reproductive|male reproductive|pregnancy|childbirth|puerperium|obstetrics|gynecology)\b/i },
  { system: "Hematology", pattern: /\b(?:hematology|haematology|oncology|lymphoreticular)\b/i },
  { system: "Immunology", pattern: /\b(?:allergy|immunology|immune)\b/i },
  { system: "Musculoskeletal", pattern: /\b(?:rheumatology|orthopedics|orthopaedics|sports|musculoskeletal|dermatology|skin)\b/i },
  { system: "Behavioral Science", pattern: /\b(?:psychiatric|behavioral|behavioural|substance use)\b/i },
  { system: "Microbiology", pattern: /\b(?:infectious diseases|microbiology)\b/i },
  { system: "Biostatistics and Ethics", pattern: /\b(?:biostatistics|epidemiology|population health|ethics|communication|interpersonal skills)\b/i },
]);

const DISCIPLINE_SYSTEMS = new Map([
...
function questionSubsystemText(question = {}) {
  const taxonomy = question.taxonomy && typeof question.taxonomy === "object"
    ? question.taxonomy
    : {};
  const labels = taxonomy.labels && typeof taxonomy.labels === "object"
  ...
  return [
    ...
    question.subsystemLabel,
    labels.subsystem,
    labels.subsystem_label,
    taxonomy.subsystem_label,
    taxonomy.subsystemLabel,
    question.subsystem_key,
    question.subsystemKey,
    taxonomy.subsystem_key,
    taxonomy.subsystemKey,
  ].map(clean).filter(Boolean).join(" ");
}
...
function questionDiscipline(question = {}) {
  ...
  return normalizedLabel(
    question.discipline
    || question.discipline_label
    || question.disciplineLabel
    || taxonomy.discipline
    || taxonomy.discipline_label
    || taxonomy.disciplineLabel,
  );
}
...
export function classifyStep1DiagnosticQuestion(question = {}) {
  ...
  const subsystemText = questionSubsystemText(question);
  const subsystemSystem = SUBSYSTEM_RULES.find((rule) => rule.pattern.test(subsystemText))?.system;
  if (subsystemSystem) return subsystemSystem;
  const text = questionText(question);
  const titleSystem = TITLE_RULES.find((rule) => rule.pattern.test(text))?.system;
  if (titleSystem) return titleSystem;
  return DISCIPLINE_SYSTEMS.get(questionDiscipline(question)) || null;
}
...
function balancedDiagnosticQuestions(grouped, requestedCount, {
  selectionSeed = "",
  questionExposureCounts = {},
  systemOrder = STEP_1_SYSTEMS,
} = {}) {
  const orderedSystems = Array.isArray(systemOrder) ? systemOrder : STEP_1_SYSTEMS;
  const availableSystemKeys = orderedSystems
    ...
    .sort((left, right) => (
      selectionSeed
        ...
        : orderedSystems.indexOf(left) - orderedSystems.indexOf(right)
    ));
...
function diagnosticTargetQuestionCounts(systems = [], requestedCount = 0, targetPercentBySystem = {}) {
  const weights = systems.map((system) => Math.max(0, Number(targetPercentBySystem?.[system]) || 0));
  const totalWeight = weights.reduce((sum, value) => sum + value, 0);
  if (!totalWeight) return null;
  const requested = Math.max(1, Math.trunc(Number(requestedCount) || 1));
  const rows = systems.map((system, index) => {
    const exact = (weights[index] / totalWeight) * requested;
    return { system, index, exact, count: Math.floor(exact), remainder: exact - Math.floor(exact) };
  });
  let remaining = requested - rows.reduce((sum, row) => sum + row.count, 0);
  ...
  if (requested >= systems.length) {
    for (const empty of rows.filter((row) => row.count === 0)) {
      const donor = [...rows]
  ...
  return Object.fromEntries(rows.map((row) => [row.system, row.count]));
}

export function buildMultiExamDiagnosticSelection(candidates = [], {
  requestedCount = 40,
  minimumSystems = 6,
  allowedSystems = [],
  resolveSystem = () => "",
  ...
  selectionSeed = "",
  questionExposureCounts = {},
  targetPercentBySystem = {},
} = {}) {
  const safeCount = Math.max(1, Math.min(200, Math.trunc(Number(requestedCount) || 40)));
  const systems = [...new Set((Array.isArray(allowedSystems) ? allowedSystems : []).map(clean).filter(Boolean))];
  ...
  for (const question of Array.isArray(candidates) ? candidates : []) {
    ...
    const system = clean(resolveSystem(question));
    if (!system || !grouped.has(system)) {
      rejected.unclassified.push({ id: String(question.id), title: clean(question.title) });
      continue;
    }
    const media = auditDiagnosticQuestionMedia(question);
    mediaReferenceCount += media.referenceCount;
    const profile = { ...diagnosticQuestionMatchProfile(question), system };
    if (!media.ready) {
  ...
  const requestedPreferredIds = [...new Set(
    (Array.isArray(preferredQuestionIds) ? preferredQuestionIds : []).map(clean).filter(Boolean),
  )].slice(0, safeCount);
  const selectedRecords = [];
  const selectedIds = new Set();
  for (const id of requestedPreferredIds) {
    const record = recordsById.get(id);
    if (!record) rejected.preferredUnavailable.push({ id });
    else {
      selectedRecords.push(record);
      selectedIds.add(id);
    }
  }
  const targetQuestionCountBySystem = diagnosticTargetQuestionCounts(
    systems,
    safeCount,
    targetPercentBySystem,
  );
  if (targetQuestionCountBySystem) {
    for (const system of systems) {
      const alreadySelected = selectedRecords.filter((record) => record.profile.system === system).length;
      const required = Math.max(0, Number(targetQuestionCountBySystem[system] || 0) - alreadySelected);
      const queue = selectionSeed
        ? diverseDiagnosticSystemQueue(grouped.get(system) || [], {
          selectionSeed,
          system,
          questionExposureCounts,
      ...
      for (const record of queue) {
        if (added >= required || selectedRecords.length >= safeCount) break;
        if (selectedIds.has(record.profile.id)) continue;
        selectedIds.add(record.profile.id);
        selectedRecords.push(record);
        added += 1;
      }
    }
  ...
  for (const record of balancedDiagnosticQuestions(grouped, safeCount, {
    ...
    if (selectedRecords.length >= safeCount) break;
    if (selectedIds.has(record.profile.id)) continue;
    selectedIds.add(record.profile.id);
    selectedRecords.push(record);
  }

  const selected = selectedRecords.map((record) => record.question);
  const selectedProfiles = selectedRecords.map((record) => record.profile);
  const availableSystemKeys = systems.filter((system) => grouped.get(system)?.length > 0);
  const selectedSystemKeys = systems.filter((system) => selected.some((question) => question.diagnostic_system === system));
  const selectedQuestionCountBySystem = Object.fromEntries(systems.map((system) => [
    system,
    selected.filter((question) => question.diagnostic_system === system).length,
  ]));
  const weightingReady = !targetQuestionCountBySystem || systems.every((system) => (
    Number(selectedQuestionCountBySystem[system] || 0)
      === Number(targetQuestionCountBySystem[system] || 0)
  ));
  const requiredSystems = Math.max(1, Math.min(
    safeCount,
    systems.length,
    Math.trunc(Number(minimumSystems) || 1),
  ));
  const taxonomyDepthReady = selectedProfiles.every((profile) => Boolean(profile.system) && Boolean(profile.topic));
  const distinctProfileValues = (field) => new Set(selectedProfiles.map((profile) => profile[field]).filter(Boolean)).size;
  ...
  const studentAttemptSeeded = Boolean(clean(selectionSeed) && requestedPreferredIds.length === 0);

  return {
    ready: selected.length === safeCount
    ...
    eligibleQuestionCount: records.length,
    availableSystemKeys,
    selectedSystemKeys,
    targetPercentBySystem: targetQuestionCountBySystem ? { ...targetPercentBySystem } : null,
    targetQuestionCountBySystem,
    selectedQuestionCountBySystem,
    weightingReady,
    minimumSystems: requiredSystems,
    mediaReferenceCount,
    selectedImageQuestionCount: selectedRecords.filter((record) => record.media.hasPlayableImage).length,
    diagnosticTaxonomyByQuestionId: Object.fromEntries(selectedProfiles.map((profile) => [profile.id, profile])),
    selectionMode: studentAttemptSeeded ? "student_attempt_seeded" : requestedPreferredIds.length ? "governed_fixed_set_preview" : "deterministic_legacy",
    selectionSeedHash: clean(selectionSeed) ? crypto.createHash("sha256").update(clean(selectionSeed)).digest("hex") : null,
...
export function applyDiagnosticSystemOverride(question = {}, system = "", { allowApprovedLabel = false } = {}) {
  const canonical = allowApprovedLabel
    ? (diagnosticMatchLabel(system) ? clean(system) : "")
...
export function diagnosticSessionUsesCurrentBlueprint(session = {}) {
  ...
  if (
    ...
    || (quality.weightingRequired === true && quality.weightingReady !== true)
    || (quality.officialBlueprintRequired === true && !String(quality.officialBlueprintId || "").trim())
    || (quality.officialBlueprintRequired === true
      && String(quality.examVariant || "") !== String(session.nclexVariant || session.nclex_variant || ""))
  ) {
    return false;
  }
  const sessionExam = normalizedLabel(session.examTrack || session.exam_track || session.examTrackId || session.exam_track_id);
  const step1Session = sessionExam.includes("step 1") || sessionExam === "usmle step 1";
  const mappedSystems = mappings.map((mapping) => {
    const value = systemMap[String(mapping.contentQuestionId || "")];
    return step1Session
      ? canonicalStep1DiagnosticSystem(value)
      : diagnosticMatchLabel(value) ? clean(value) : "";
  });
