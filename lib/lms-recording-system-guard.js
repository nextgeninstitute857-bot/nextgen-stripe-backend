export const LMS_RECORDING_SYSTEM_GUARD_BUILD = "v300-explicit-override-preserve";
...
// A raw Zoom title is useful evidence, but it is not always authoritative:
// the recorded meeting can be deliberately moved into the corrected schedule.
// Only an admin's explicit selection of the *same* existing session can lock
// that exception. This keeps the automatic guard strict for every ordinary or
// cross-session assignment while preserving a reviewed correction on restart.
export function preservesExplicitSystemOverride(recording = {}, session = {}) {
  const recordedSystem = explicitRecordingSystem(recording.system || recording.topic);
  const sessionSystem = explicitRecordingSystem(session.system || session.topic || session.title);
  return recording.assignment_locked === true &&
    recording.assignment_source === "admin_explicit_session" &&
    clean(recording.session_id) === clean(session.id) &&
    Boolean(recordedSystem) &&
    recordedSystem === sessionSystem;
}
...
export function reconcilePublishedRecordingSystemMismatches(db = {}, zoomMeetings = [], {
  ...
  for (const [storageKey, recording] of Object.entries(db.recordings || {})) {
    ...
    if (preservesExplicitSystemOverride(recording, session)) continue;
