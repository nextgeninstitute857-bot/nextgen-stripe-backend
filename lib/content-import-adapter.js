...
export function extractMediaReferences(...values) {
  ...
  for (const value of values) {
    ...
    for (const candidate of candidates) {
      const clean = String(candidate || "").trim().replace(/^[([{]+|[\])}.!?]+$/g, "").split(/[?#]/)[0];
      let decoded = clean;
      try { decoded = decodeURIComponent(clean); } catch { /* Keep the original safe token. */ }
      const name = path.basename(decoded);
      const extension = path.extname(name).slice(1).toLowerCase();
      if (name && CONTENT_MEDIA_EXTENSIONS.has(extension)) found.add(name);
    }
  }
  return [...found];
}
...
export function adaptUniversalQuestion(question, answers, context = {}) {
  ...
  const considerMedia = (name, placement, priority) => {
    const semanticKey = mediaMatchKeys(name).at(-1) || String(name).toLowerCase();
    const score = priority + (/^highresdefault_/i.test(path.basename(name)) ? 10 : 0);
    const current = preferredMedia.get(semanticKey);
    if (!current || score > current.score) preferredMedia.set(semanticKey, { name, placement, score });
  };
