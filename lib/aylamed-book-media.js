...
export function authorizedPrivateBookResources(resources = []) {
  return (Array.isArray(resources) ? resources : []).filter((resource) => {
    ...
    return type === "book" && access === "protected" && VERIFIED_RIGHTS.has(rights);
  });
}

export function authorizedBookMediaResources(resources = [], bookKey = "") {
  const wanted = cleanString(bookKey, 180).toLowerCase();
  return authorizedPrivateBookResources(resources).filter((resource) => (
    cleanString(resource.folderId || resource.folder_id, 180).toLowerCase() === wanted
  ));
}
