...
export function normalizeContentTaxonomyProviderPairClassification(proposal = {}, request = {}) {
  ...
  const proposedOutlierQuestionIds = [...new Set(
    (Array.isArray(proposal.outlier_question_ids) ? proposal.outlier_question_ids : [])
      .map((id) => cleanText(id, 80))
      .filter(Boolean),
  )].slice(0, CONTENT_TAXONOMY_CLASSIFIER_EVIDENCE_LIMIT);
  const unknownOutlierQuestionIds = proposedOutlierQuestionIds.filter(id => !suppliedQuestionIds.has(id));
  const outlierQuestionIds = proposedOutlierQuestionIds.filter(id => suppliedQuestionIds.has(id));
  ...
  const autoApprovalReady = Boolean(
    request.evidenceComplete === true
      ...
      && unknownOutlierQuestionIds.length === 0
  ...
  if (unknownOutlierQuestionIds.length) reviewReasons.push("unknown_outlier_question_ids");
