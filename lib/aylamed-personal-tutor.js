...
function verifiedQuestionSignals(questionAttempts = []) {
  const rows = (Array.isArray(questionAttempts) ? questionAttempts : [])
    .filter((row) => row && (row.serverVerified === true || String(row.sourceType || row.source_type || "").toLowerCase() === "external"))
...
function crossSystemWeakPattern({ questionAttempts = [], flashcardReviews = [], conceptMastery = [] } = {}) {
  ...
  for (const row of Array.isArray(questionAttempts) ? questionAttempts : []) {
    if (!(row?.serverVerified === true || String(row?.sourceType || row?.source_type || "").toLowerCase() === "external")) continue;
...
function recommendation({ planIdentity, kind, title, reason, planChange = false, directive = null, returnLink = null }) {
  return {
