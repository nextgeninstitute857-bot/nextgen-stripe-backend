...
function modalityForAssignment(assignment = {}) {
  ...
  if (["diagnostic", "baseline_diagnostic"].includes(category)) return "complete";
...
function safeNextAction(assignment, planIdentity) {
  ...
  const category = cleanText(assignment.category || assignment.type, 80).toLowerCase();
  const reasons = [];
  ...
  return {
    ...
    reason: reasons.join("; "),
    returnLink: ["diagnostic", "baseline_diagnostic"].includes(category)
      ? "/qbank?diagnostic=1"
      : `/roadmap/assignments/${encodeURIComponent(String(assignment.id || ""))}`,
    actionTarget: ["diagnostic", "baseline_diagnostic"].includes(category)
      ? actionTarget("diagnostic")
      : actionTarget("roadmap_assignment", { assignmentId: assignment.id }),
...
function readinessPaceGuidance({
  estimate = null,
  student = {},
  questionSignals = {},
  assessments = {},
  ...
  if (!targetDate) {
    return {
      state: "target_date_required",
      message: "Add an exam date before AylaMed estimates a readiness timeline.",
      projectedWorkloadDays: null,
      nextCheckpointDays: null,
      passPrediction: false,
    };
  }
  if (student.serverVerifiedBaseline !== true && Number(assessments.count || 0) < 1) {
    return {
      state: "verified_baseline_required",
      message: "Complete the verified diagnostic before AylaMed gives a readiness timeline.",
      projectedWorkloadDays: null,
      nextCheckpointDays: null,
      passPrediction: false,
    };
  ...
  if (!estimate || typeof estimate !== "object") {
    return {
      ...
      message: "A verified readiness estimate is temporarily unavailable; the stored roadmap remains unchanged.",
      projectedWorkloadDays: null,
      nextCheckpointDays: null,
      passPrediction: false,
    };
  ...
  const estimatedHours = numberBetween(estimate.workload?.estimatedHours, 0, 100000, 0);
  const dailyHours = numberBetween(
    estimate.inputs?.dailyHoursAvailable
      ?? student.dailyHours
      ?? student.daily_hours,
    0,
    16,
  ...
  const weeklyStudyDays = Math.round(numberBetween(
    estimate.inputs?.weeklyStudyDays
      ?? student.weeklyStudyDays
      ?? student.weekly_study_days,
    1,
    7,
    6,
  ));
  if (!estimatedHours || !dailyHours) {
    return {
      state: "available_hours_required",
      message: "Add realistic available study hours before AylaMed estimates a readiness timeline.",
      projectedWorkloadDays: null,
      nextCheckpointDays: null,
      passPrediction: false,
    };
  ...
  const recent = recentPlanSignals(recentPlans);
  const observedPace = recent.observedDays >= 3
    && Number.isFinite(Number(recent.averageCompletionPercent));
  const adherenceFactor = observedPace
    ? numberBetween(Number(recent.averageCompletionPercent) / 100, 0.35, 1.1, 1)
    : 1;
  const effectiveWeeklyHours = dailyHours * weeklyStudyDays * adherenceFactor;
  const projectedWorkloadDays = Math.max(
    1,
    Math.ceil((estimatedHours / effectiveWeeklyHours) * 7),
  );
  const targetDays = Math.max(
    1,
    Math.round(numberBetween(estimate.exam?.targetDays, 1, 3650, projectedWorkloadDays)),
  );
  const nextMilestone = (Array.isArray(estimate.milestones) ? estimate.milestones : [])
    .filter((row) => Number(row?.byDay) > 0)
    .sort((left, right) => Number(left.byDay) - Number(right.byDay))[0] || null;
  const nextCheckpointDays = nextMilestone
    ? Math.max(1, Math.round(numberBetween(nextMilestone.byDay, 1, 3650, 1)))
    : null;
  const onSchedule = projectedWorkloadDays <= targetDays;
  const basis = observedPace ? "observed_completion" : "entered_availability";
  const evidenceLabel = Number(assessments.count || 0) > 0
    ? "verified assessment evidence"
    : `${Math.max(0, Number(questionSignals.count || 0))} verified question result${Number(questionSignals.count || 0) === 1 ? "" : "s"}`;
  const paceLabel = observedPace
    ? `${recent.averageCompletionPercent}% average completion across ${recent.observedDays} recent days`
    : `${dailyHours} available hour${dailyHours === 1 ? "" : "s"} on ${weeklyStudyDays} day${weeklyStudyDays === 1 ? "" : "s"} each week`;
  const message = onSchedule
    ? `Good progress. Based on ${evidenceLabel} and ${paceLabel}, the modeled workload fits in about ${projectedWorkloadDays} calendar days at this pace. The next verified assessment will refresh this estimate.`
    : `Keep going, but the current pace needs adjustment: the modeled workload is about ${projectedWorkloadDays} calendar days, compared with ${targetDays} days to the entered exam date. AylaMed should reduce low-value coverage, increase available time, or revisit the date after the next verified assessment.`;

  return {
    state: onSchedule ? "on_schedule" : "pace_adjustment_needed",
    confidence: cleanText(estimate.confidence?.level || "low", 40).toLowerCase(),
    basis,
    observedDays: recent.observedDays,
    averageCompletionPercent: recent.averageCompletionPercent,
    projectedWorkloadDays,
    targetDays,
    daysBeyondTarget: Math.max(0, projectedWorkloadDays - targetDays),
    nextCheckpointDays,
    nextCheckpointTitle: cleanText(nextMilestone?.title, 240) || null,
    message,
    passPrediction: false,
    disclaimer: "This is a workload-planning estimate, not a prediction or guarantee of passing. Verified assessment evidence takes priority.",
  };
}
...
export function buildAylaPersonalTutorDecision(input = {}) {
  ...
  const readinessPace = readinessPaceGuidance({
    estimate: input.pathwayEstimate,
    student,
    questionSignals,
    assessments,
    recentPlans: input.recentPlans,
  ...
  if (["on_schedule", "pace_adjustment_needed"].includes(readinessPace.state)) {
    recommendations.push(recommendation({
      planIdentity,
      kind: "readiness_pace",
      title: readinessPace.state === "on_schedule"
        ? `Keep this pace: modeled workload about ${readinessPace.projectedWorkloadDays} days`
        : "Adjust the pace or exam plan",
      reason: `${readinessPace.message} ${readinessPace.disclaimer}`,
      returnLink: "/roadmap",
      target: actionTarget("roadmap"),
    }));
  }
...
function aylaTutorSentence(value = "") {
...
export function formatAylaPersonalTutorAnswer(decision = {}) {
  ...
  const parts = [];
  if (next.assignmentId) parts.push(`Next, ${next.modality || "study"}: ${aylaTutorSentence(next.title)} ${aylaTutorSentence(next.reason)}`.trim());
  ...
  if (decision.nbmeReadiness?.recommendation?.title) {
    parts.push(`${aylaTutorSentence(decision.nbmeReadiness.recommendation.title)} ${aylaTutorSentence(decision.nbmeReadiness.recommendation.reason)}`.trim());
  }
  if (["on_schedule", "pace_adjustment_needed"].includes(decision.readinessPace?.state)) {
    parts.push(`${decision.readinessPace.message} ${decision.readinessPace.disclaimer}`);
  }
