...
function rawAssetTitle(value = "") {
  const clean = cleanString(value, 240);
  if (!clean) return false;
  const basename = clean.replace(/\\/g, "/").split("/").at(-1) || clean;
  const stem = basename.replace(/\.[a-z0-9]{2,6}$/i, "");
  return clean.includes("/")
    || clean.includes("\\")
    || /\.[a-z0-9]{2,6}$/i.test(basename)
    || /^[a-z]*\d+[a-z0-9_-]*$/i.test(stem);
}

function inferredTitle(input = {}) {
  const explicit = cleanString(input.title || input.video_title, 240);
  if (explicit && !rawAssetTitle(explicit)) return explicit;
  const qbankTaxonomy = input.qbankTaxonomy || input.qbank_taxonomy || {};
  const mappedTopic = cleanString(
    input.topic
      || input.topicKey
      || input.topic_key
      || qbankTaxonomy.topicKey
      || qbankTaxonomy.topic_key
      || input.subtopic
      || input.subtopicKey
      || input.subtopic_key
  ...
  if (mappedTopic && !rawAssetTitle(mappedTopic) && !/^\d+$/.test(mappedTopic)) return mappedTopic;
  const original = cleanString(input.originalName || input.original_name, 240).replace(/\.[a-z0-9]{2,6}$/i, "");
  if (original && !rawAssetTitle(original) && !/^\d+$/.test(original)) {
    return original.replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
  }
  return "Focused video review";
}
...
export function normalizeAylaContentHubVideo(input = {}, { examTrack = null } = {}) {
  ...
  const supplementalForExamTrackId = normalizeAylaShellExamTrack(
    input.supplementalForExamTrackId || input.supplemental_for_exam_track_id,
  );
  ...
  const visibleExamTrackId = requestedExamTrackId && supplementalForExamTrackId === requestedExamTrackId
    ? requestedExamTrackId
    : examTrackId;
  if (!resourceId || !examTrackId || (requestedExamTrackId && requestedExamTrackId !== visibleExamTrackId)) return null;
  ...
  const sourceData = input.sourceData || input.source_data || {};
  const nestedSource = sourceData.source && typeof sourceData.source === "object" ? sourceData.source : {};
  const folderId = cleanString(
    input.folderId || input.folder_id || input.sourceFolderId || input.source_folder_id
      || sourceData.folderId || sourceData.folder_id || sourceData.projectId || sourceData.project_id
      || nestedSource.folderId || nestedSource.folder_id || nestedSource.projectId || nestedSource.project_id,
    180,
  );
  const folderName = cleanString(
    input.folderName || input.folder_name || input.sourceFolderName || input.source_folder_name
      || sourceData.folderName || sourceData.folder_name || sourceData.vimeoFolderName || sourceData.vimeo_folder_name
      || sourceData.projectName || sourceData.project_name
      || nestedSource.folderName || nestedSource.folder_name || nestedSource.projectName || nestedSource.project_name,
    240,
  );
  const explicitPlaylistKey = cleanString(folderId || folderName || input.playlistKey || input.playlist_key, 180);
  ...
  return {
    ...
    sourceType,
    type: "vimeo_video",
    examTrackId: visibleExamTrackId,
    examTrack: normalizeAylaRegistryExamTrack(visibleExamTrackId),
    sourceExamTrackId: examTrackId,
    supplementalForExamTrackId: supplementalForExamTrackId || null,
    supplemental: Boolean(supplementalForExamTrackId),
    scoringAllowed: !supplementalForExamTrackId,
    ...
    playlistKey,
    playlistTitle: folderName || cleanString(input.playlistTitle || input.playlist_title, 240) || system,
...
export function scopeAylaContentHubVideoForExam(input = {}, examTrack = "") {
  const targetExamTrackId = normalizeAylaShellExamTrack(examTrack);
  const sourceExamTrackId = normalizeAylaShellExamTrack(
    input.sourceExamTrackId || input.source_exam_track_id
      || input.examTrackId || input.exam_track_id || input.examTrack || input.exam_track || input.exam,
  );
  if (!targetExamTrackId || !sourceExamTrackId) return { ...input };
  const supplemental = sourceExamTrackId !== targetExamTrackId;
  return {
    ...input,
    sourceExamTrackId,
    supplementalForExamTrackId: supplemental ? targetExamTrackId : null,
    supplemental,
    scoringAllowed: !supplemental,
  };
}
...
export function sanitizeAylaContentHubVideo(video = {}, { progress = null, assignment = null } = {}) {
  return {
    ...
    source_exam_track_id: video.sourceExamTrackId || video.examTrackId,
    supplemental: video.supplemental === true,
    supplemental_label: video.supplemental === true ? "Step 2 CK Supplemental" : null,
    scoring_allowed: video.supplemental === true ? false : video.scoringAllowed !== false,
    system: video.system,
...
export function buildAylaContentHubCatalog({
  ...
  const filtered = available.filter((video) => {
    ...
    if (subsystem && cleanKey(video.subsystem || "Unclassified", "unclassified") !== subsystem) return false;
    ...
    if (search && !`${video.title} ${video.description} ${video.playlistTitle} ${video.system} ${video.subsystem} ${video.topic} ${video.subtopic} ${video.topicAliases.join(" ")}`.toLowerCase().includes(search)) return false;
    return true;
  });
  ...
  for (const video of filtered) {
    const row = playlists.get(video.playlistKey) || {
      key: video.playlistKey,
    ...
    row.video_count += 1;
    playlists.set(video.playlistKey, row);
  }
  const facets = {
  ...
  return {
    ...
    playlists: [...playlists.values()].sort((left, right) => left.title.localeCompare(right.title, undefined, { numeric: true })).map((row) => ({
      ...row,
...
export function selectAylaRoadmapVideo({
  ...
  const candidateRows = normalizeAylaContentHubVideos(videos, { examTrack })
  ...
  // If the catalogue has a verified lecture for the requested subsystem,
  // stay inside that boundary even when another subsystem happens to reuse
  // the same topic name. Otherwise allow a verified system-wide fallback;
  // QBank taxonomies are often more specific than the Vimeo catalogue.
  const hasSubsystemMatch = Boolean(subsystemKey && candidateRows.some((row) => row.subsystemMatch));
  const candidates = candidateRows
    .filter((row) => !hasSubsystemMatch || row.subsystemMatch)
