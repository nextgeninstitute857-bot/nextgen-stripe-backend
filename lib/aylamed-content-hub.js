...
export function normalizeAylaContentHubVideo(input = {}, { examTrack = null } = {}) {
  const sourceType = cleanString(input.sourceType || input.source_type, 40).toLowerCase() === "registry" ? "registry" : "legacy";
  ...
  return {
    ...
    system,
    topic,
    subtopic: cleanString(input.subtopic || input.subtopicKey || input.subtopic_key, 180),
    playlistKey,
...
export function normalizeAylaContentHubVideos(videos = [], { examTrack = null } = {}) {
  ...
  return [...byProvider.values()].sort((left, right) =>
    String(left.playlistTitle).localeCompare(String(right.playlistTitle))
...
export function buildAylaContentHubCatalog({
  ...
  const filtered = available.filter((video) => {
    ...
    if (topic && cleanKey(video.topic, "") !== topic) return false;
    if (playlist && cleanKey(video.playlistKey, "") !== playlist) return false;
    if (search && !`${video.title} ${video.description} ${video.system} ${video.topic} ${video.subtopic}`.toLowerCase().includes(search)) return false;
    return true;
  });
  ...
  return {
    ...
    filters: { system: system || null, topic: topic || null, playlist: playlist || null, search: search || null },
    facets,
    continue_watching: serialized.filter((video) => video.progress.watched_percent > 0 && !video.progress.completed),
    playlists: [...playlists.values()].map((row) => ({ ...row, topics: [...row.topics].sort(), video_count: row.videos.length })),
    videos: serialized,
  };
}
...
export function selectAylaRoadmapVideo({
  ...
  const candidates = normalizeAylaContentHubVideos(videos, { examTrack })
    ...
    .map((video) => {
      ...
      const topicMatch = Boolean(topicKey && cleanKey(video.topic, "") === topicKey);
      ...
      return { video, progress, completed, systemMatch, topicMatch, resume, score };
    })
    .filter((row) => !row.completed && (!systemKey || row.systemMatch || row.topicMatch))
