...
const PURPOSES = new Set(["question_zip", "media_zip", "image_zip", "video_zip"]);
...
function cleanFilename(value) {
  ...
  if (!/\.zip$/i.test(clean)) throw statusError("Only ZIP uploads are accepted");
  return clean;
}
...
export class CloudContentUploadStore {
  ...
  async create({ originalFilename, totalBytes, expectedSha256 = "", purpose = "question_zip", createdBy = "", metadata = {} } = {}) {
    ...
    if (bytes > this.maxUploadBytes) throw statusError("ZIP exceeds the configured 50 GB direct-upload limit", 413);
    const cleanPurpose = String(purpose || "question_zip").trim().toLowerCase();
    if (!PURPOSES.has(cleanPurpose)) throw statusError("purpose must be question_zip or media_zip");
    ...
    const created = await createContentR2Multipart({ objectKey, metadata: {
  ...
  async resolveFinalized(id, { allowedPurposes = [] } = {}) {
    return this.withLock(id, async () => {
      ...
      if (allowedPurposes.length && !allowedPurposes.includes(session.purpose) && !(session.purpose === "media_zip" && allowedPurposes.some((purpose) => ["image_zip", "video_zip", "media_zip"].includes(purpose)))) {
      ...
      if (!head || head.sizeBytes !== Number(session.total_bytes)) throw statusError("Finalized R2 ZIP is no longer available", 410);
      session.updated_at = this.now().toISOString();
      session.expires_at = new Date(this.now().getTime() + this.sessionTtlMs).toISOString();
      await this.write(session);
      return {
