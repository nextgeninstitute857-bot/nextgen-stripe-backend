...
import { NEXTGEN_BRAND_ID } from "./crm-brand-routing.js";
...
export function buildMetaPerformanceSnapshot(snapshot, brandId) {
...
export function saveMetaPerformanceSnapshot(db, snapshot, brandId) {
  const performance = buildMetaPerformanceSnapshot(snapshot, brandId);
  db.meta_ads_performance_by_brand = {
    ...(db.meta_ads_performance_by_brand || {}),
    [brandId]: performance,
  };
  // Keep the legacy singleton pinned to NextGen once both brands exist. Older
  // jobs and screens continue to work while brand-aware consumers use the map.
  if (!db.meta_ads_performance || db.meta_ads_performance.brand_id === brandId || brandId === NEXTGEN_BRAND_ID) {
    db.meta_ads_performance = performance;
  }
  return performance;
}
...
export function preserveMetaReportingForLegacyWrite(current = {}, incoming = {}) {
  if (!current.meta_ads_sync_state && !current.meta_ads_performance && !current.meta_ads_performance_by_brand) return {};
  const protectedFields = {};
  for (const key of ["meta_ads_sync_state", "meta_ads_performance", "meta_ads_performance_by_brand", "meta_ads_last_sync", "meta_ads_last_sync_by_brand", "meta_ads_rollup_archive", "meta_ads_brand_reconciliation_archive"]) {
    if (Object.hasOwn(current, key)) protectedFields[key] = current[key];
  }
  const currentSyncTimes = [
    current.meta_ads_performance?.synced_at,
    ...Object.values(current.meta_ads_performance_by_brand || {}).map((row) => row?.synced_at),
  ].filter(Boolean).sort();
  const incomingSyncTimes = [
    incoming.meta_ads_performance?.synced_at,
    ...Object.values(incoming.meta_ads_performance_by_brand || {}).map((row) => row?.synced_at),
  ].filter(Boolean).sort();
  if (currentSyncTimes.length && currentSyncTimes.join("|") !== incomingSyncTimes.join("|")) {
