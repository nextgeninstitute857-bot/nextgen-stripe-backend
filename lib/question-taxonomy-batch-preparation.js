...
export function prepareQuestionTaxonomyBatch(evidence, options = {}) {
  ...
  for (const question of evidence.questions) {
    ...
    const documents = { stem: separateClinicalHtml(question.question_html), explanation: separateClinicalHtml(question.explanation_html),
      choices: question.answers.map(answer => ({ answer_id: answer.answer_id, ...separateClinicalHtml(answer.text_html) })) };
