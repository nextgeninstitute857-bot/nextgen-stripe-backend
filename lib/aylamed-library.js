...
import { normalizeOriginalBookPdf } from "./aylamed-book-pdf.js";
...
function cleanCoverImageUrl(value = "") {
...
function normalizeReaderMediaReferences(page = {}) {
  const rows = page.mediaReferences || page.media_references;
  return values(rows).slice(0, 250).map((item, index) => ({
    id: cleanString(item.id || item.referenceId || item.reference_id || `figure-${index + 1}`, 160),
    ref: cleanString(item.ref || item.mediaRef || item.media_ref, 1_000),
    matchPaths: cleanList(item.matchPaths || item.match_paths).slice(0, 12),
    alt: cleanString(item.alt || item.label, 500),
    caption: cleanString(item.caption, 1_000),
    placement: cleanString(item.placement || "inline", 80).toLowerCase() || "inline",
    order: Math.max(0, Number(item.order ?? index) || 0),
  })).filter((item) => item.ref);
}

function normalizeReaderMedia(page = {}) {
  return values(page.media || page.mediaAssets || page.media_assets).slice(0, 250).map((item, index) => ({
    id: cleanString(item.id || `figure-${index + 1}`, 160),
    ref: cleanString(item.ref, 1_000),
    alt: cleanString(item.alt, 500),
    caption: cleanString(item.caption, 1_000),
    placement: cleanString(item.placement || "inline", 80).toLowerCase() || "inline",
    order: Math.max(0, Number(item.order ?? index) || 0),
    kind: "image",
    objectKey: cleanString(item.objectKey || item.object_key, 1_000),
    contentType: cleanString(item.contentType || item.content_type || "image/*", 120),
...
function normalizeReaderPages(input = {}) {
  ...
  const rows = rawReaderPages(input).slice(0, MAX_READER_PAGES).map((page, index) => {
    ...
    return {
      ...
      mediaReferences: normalizeReaderMediaReferences(page),
      media: normalizeReaderMedia(page),
    };
  ...
  const pages = [...grouped.values()].map((page) => {
    ...
    const mediaReferences = parts.flatMap((part) => part.mediaReferences || []);
    const media = parts.flatMap((part) => part.media || [])
...
export function normalizeAylaLibraryResource(input = {}, { examTrack = null } = {}) {
  ...
  return {
    ...
    bookTitle,
    coverImageUrl: cleanCoverImageUrl(input.coverImageUrl || input.cover_image_url),
    ...
    readerPages: pages,
    sourcePdf: normalizeOriginalBookPdf(input),
...
export function sanitizeAylaLibraryResource(resource = {}, { studentId = "", progress = null, assignment = null } = {}) {
  ...
  const sourceVisible = resource.sourceLabelVisible === true && Boolean(resource.sourceLabel);
  const originalPdfAvailable = Boolean(resource.sourcePdf?.objectKey && resource.sourcePdf?.sizeBytes);
  ...
  return {
    ...
    cover_image_url: resource.coverImageUrl || null,
    ...
    reader: {
      mode: originalPdfAvailable ? "original_pdf_page_turn" : "page_turn",
      ...
      original_pdf_available: originalPdfAvailable,
      original_pdf_path: originalPdfAvailable ? `${basePath}/original.pdf${query}` : null,
      original_pdf_start_page: resource.pdfPageStart || firstPage?.pdfPage || 1,
    },
    progress: summary,
...
export function buildAylaLibraryReader(resourceInput = {}, {
  ...
  return {
    ...
    navigation: {
      mode: resource.sourcePdf ? "original_pdf_page_turn" : "page_turn",
      continuous_pdf_scroll: false,
      original_pdf_available: Boolean(resource.sourcePdf),
    },
  };
}
...
export function buildAylaLibraryPage(resourceInput = {}, selector = "", {
  ...
  return {
    ...
    page: {
      ...
      exact_reference: resource.sourceLabelVisible ? page.exactReference || resource.exactReference || "" : "",
      content: page.text,
      media_count: Array.isArray(page.media) ? page.media.length : 0,
      read: read.has(page.key),
    },
    navigation: {
      mode: "page_turn",
...
function librarySearchSnippet(text = "", token = "", radius = 110) {
  const normalized = String(text || "").replace(/\s+/g, " ").trim();
  const index = normalized.toLowerCase().indexOf(String(token || "").toLowerCase());
  if (index < 0) return normalized.slice(0, radius * 2);
  const start = Math.max(0, index - radius);
  const end = Math.min(normalized.length, index + String(token).length + radius);
  return `${start > 0 ? "…" : ""}${normalized.slice(start, end)}${end < normalized.length ? "…" : ""}`;
}

export function searchAylaLibraryPages(resourceInput = {}, query = "", {
  examTrack = null,
  studentId = "",
  assignment = null,
  limit = 20,
  ...
  const resource = normalizeAylaLibraryResource(resourceInput, { examTrack });
  const cleanQuery = cleanString(query, 180).replace(/\s+/g, " ");
  if (!resource?.readerReady || cleanQuery.length < 2) {
    return { query: cleanQuery, total: 0, results: [] };
  }
  const tokens = [...new Set(
    cleanQuery.toLowerCase().split(/\s+/).filter((token) => token.length >= 2),
  )];
  if (!tokens.length) return { query: cleanQuery, total: 0, results: [] };
  const safeLimit = Math.max(1, Math.min(25, Math.trunc(Number(limit) || 20)));
  const basePath = resourceBasePath(studentId, resource.id);
  const querySuffix = assignmentQuery(assignment);
  const matches = resource.readerPages.flatMap((page, index) => {
    const heading = resource.sourceLabelVisible
      ? page.heading || ""
      : sanitizeAylaHiddenSourceText(page.heading, resource, "");
    const searchable = `${heading} ${page.text}`.toLowerCase();
    if (!tokens.every((token) => searchable.includes(token))) return [];
    return [{
      page_key: page.key,
      book_page: page.turnNumber || index + 1,
      heading,
      snippet: librarySearchSnippet(page.text, tokens[0]),
      page_path: `${basePath}/pages/${pagePathKey(page)}${querySuffix}`,
    }];
  });
  return {
    query: cleanQuery,
