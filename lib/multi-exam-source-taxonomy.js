...
// These PLAB ledgers are the provider's own discipline groupings as preserved
// in the authorized source exports. They deliberately resolve only to the
// controlled PLAB systems used by AylaMed; an unknown provider ID is left
// unresolved so it cannot silently become medically reviewed content.
const PLAB_BMJ_DISCIPLINES = Object.freeze({
  5648: ["Medicine", "Haematology"],
  5649: ["Surgery", "Breast Surgery"],
  5650: ["Medicine", "Cardiology"],
  5651: ["Pediatrics", "Pediatrics"],
  5652: ["Medicine", "Gastroenterology and Hepatology"],
  5653: ["Medicine", "Endocrinology"],
  5654: ["Surgery", "Ear, Nose and Throat"],
  5655: ["Clinical Ethics", "Ethics, Law and Professionalism"],
  5656: ["Surgery", "Ophthalmology"],
  5657: ["Emergency Medicine", "Emergency and Acute Care"],
  5659: ["Medicine", "Infectious Diseases"],
  5660: ["Psychiatry", "Psychiatry"],
  5661: ["Surgery", "Musculoskeletal and Orthopaedics"],
  5662: ["Medicine", "Neurology"],
  5663: ["General Practice", "Geriatrics, Palliative Care and General Practice"],
  5664: ["Emergency Medicine", "Acute Medicine"],
  5665: ["Medicine", "Renal Medicine"],
  5666: ["Obstetrics and Gynecology", "Obstetrics and Gynecology"],
  5667: ["Medicine", "Respiratory Medicine"],
  5668: ["Emergency Medicine", "Emergency Medicine"],
  5669: ["Medicine", "Dermatology"],
  5670: ["Surgery", "Urology"],
});

const PLAB_PASSMEDICINE_DISCIPLINES = Object.freeze({
  1: ["Medicine", "Dermatology"],
  2: ["Surgery", "Ear, Nose and Throat"],
  3: ["Clinical Ethics", "Ethics, Law and Professionalism"],
  4: ["Medicine", "General Medicine"],
  5: ["Surgery", "Musculoskeletal and Orthopaedics"],
  6: ["Obstetrics and Gynecology", "Obstetrics and Gynecology"],
  7: ["Surgery", "Ophthalmology"],
  8: ["Pediatrics", "Pediatrics"],
  9: ["Medicine", "Pharmacology and Therapeutics"],
  10: ["Psychiatry", "Psychiatry"],
  11: ["UK Clinical Guidelines", "Evidence, Research and Clinical Guidelines"],
  12: ["Surgery", "Surgery and Urology"],
});

const PLAB_CANADAQBANK_DISCIPLINES = Object.freeze({
  186: ["Medicine", "Medicine and General Practice"],
  187: ["Pediatrics", "Pediatrics"],
  188: ["Obstetrics and Gynecology", "Obstetrics and Gynecology"],
  189: ["Psychiatry", "Psychiatry"],
  190: ["Surgery", "Surgery and Perioperative Care"],
  191: ["Clinical Ethics", "Ethics, Population Health and Professional Practice"],
});
...
export function resolvePlabSourceDiscipline(question = {}, context = {}) {
  const provider = sourceProvider({
    ...context,
    sourceNamespace: `${context.sourceNamespace || ""} ${context.sourceFile || context.source_file || ""}`,
  });
  const nativeSysId = Number(question.sysId ?? question.native_sys_id ?? question.system_key);
  const nativeSubId = Number(question.subId ?? question.native_sub_id ?? question.subject_key);
  let mapping = null;
  let ledger = "";
  if (/bmj|onexamination|bmjone/.test(provider) || nativeSysId >= 5648 && nativeSysId <= 5670) {
    mapping = PLAB_BMJ_DISCIPLINES[nativeSysId] || null;
    ledger = "bmj_onexamination_plab_2025";
  } else if (/pass\s*medicine|passmedicine|pm[-_\s]?10/.test(provider)
    || nativeSysId === 1 && nativeSubId >= 1 && nativeSubId <= 12) {
    mapping = PLAB_PASSMEDICINE_DISCIPLINES[nativeSubId] || null;
    ledger = "passmedicine_plab_2025";
  } else if (/canada\s*qbank|canadaqbank|cqb[-_\s]?plab/.test(provider)
    || nativeSubId >= 186 && nativeSubId <= 191) {
    mapping = PLAB_CANADAQBANK_DISCIPLINES[nativeSubId] || null;
    ledger = "canadaqbank_plab_2025";
  }
  if (!mapping) {
    return {
      system: "",
      discipline: "",
      ledger,
      nativeSysId: Number.isFinite(nativeSysId) ? String(nativeSysId) : "",
      nativeSubId: Number.isFinite(nativeSubId) ? String(nativeSubId) : "",
      error: "plab_source_discipline_not_in_verified_map",
    };
  }
  return {
    system: mapping[0],
    discipline: mapping[1],
    ledger,
    nativeSysId: String(nativeSysId),
    nativeSubId: String(nativeSubId),
    error: "",
  };
}
...
function systemLabel(exam, question, context) {
  ...
  if (exam === "plab") {
    return resolvePlabSourceDiscipline(question, context).system || "PLAB Clinical Practice";
  }
...
function subsystemLabel(exam, question, context = {}) {
  const nativeSubId = Number(question.subId ?? question.native_sub_id ?? question.subject_key);
  ...
  if (exam === "plab") {
    return resolvePlabSourceDiscipline(question, context).discipline
      || clean(question.capability || question.presentation || question.subsystem)
      || "Clinical Capability";
  }
...
export function multiExamSourceQuestionTaxonomy(question = {}, context = {}) {
  ...
  const subsystem = subsystemLabel(exam, question, context);
  ...
  const plabResolution = exam === "plab" ? resolvePlabSourceDiscipline(question, context) : null;
  return {
    taxonomy: {
      system_key: `${key(exam)}:${key(system)}`,
      ...
      labels: { system, subsystem, topic, subtopic },
      source: MULTI_EXAM_SOURCE_TAXONOMY_ADAPTER,
      review_status: plabResolution && !plabResolution.error
        ? "source_evidence_verified_mapping"
        : "private_draft_taxonomy_validated",
      exam_track: exam,
      native_sys_id: nativeSysId,
      native_sub_id: nativeSubId,
      clinical_task: subtopic,
      source_fingerprint: sourceFingerprint,
      ...(plabResolution && !plabResolution.error ? { source_discipline_ledger: plabResolution.ledger } : {}),
    },
    errors: plabResolution?.error ? [plabResolution.error] : [],
  };
}

export function buildPlabQuestionTaxonomyRepair(question = {}, currentTaxonomy = {}, context = {}) {
  const resolved = resolvePlabSourceDiscipline(question, context);
  if (resolved.error) return { taxonomy: null, errors: [resolved.error], resolved };
  const current = currentTaxonomy && typeof currentTaxonomy === "object" ? currentTaxonomy : {};
  const currentLabels = current.labels && typeof current.labels === "object" ? current.labels : {};
  const topic = clean(currentLabels.topic || question.title || `${resolved.discipline} Core Concepts`, 240)
    || `${resolved.discipline} Core Concepts`;
  const subtopic = clean(currentLabels.subtopic || current.clinical_task || "Clinical Reasoning", 240)
    || "Clinical Reasoning";
  const sourceFingerprint = fingerprint([
    "plab_source_discipline_repair_v1",
    resolved.ledger,
    resolved.nativeSysId,
    resolved.nativeSubId,
    resolved.system,
    resolved.discipline,
    topic,
    subtopic,
  ...
  return {
    taxonomy: {
      ...current,
      system_key: `plab:${key(resolved.system)}`,
      subsystem_key: `plab:${key(resolved.system)}:${key(resolved.discipline)}`,
      topic_key: `plab:${key(resolved.system)}:${key(topic)}`,
      subtopic_key: `plab:${key(resolved.system)}:${key(topic)}:${key(subtopic)}`,
      labels: {
        ...currentLabels,
        system: resolved.system,
        subsystem: resolved.discipline,
        topic,
        subtopic,
      },
      source: MULTI_EXAM_SOURCE_TAXONOMY_ADAPTER,
      review_status: "source_evidence_verified_mapping",
      exam_track: "plab",
      native_sys_id: resolved.nativeSysId,
      native_sub_id: resolved.nativeSubId,
      source_discipline_ledger: resolved.ledger,
      source_fingerprint: sourceFingerprint,
    },
    errors: [],
    resolved,
...
export function multiExamSourceTaxonomySummary() {
  return {
    ...
    plab_bmj_disciplines: Object.keys(PLAB_BMJ_DISCIPLINES).length,
    plab_passmedicine_disciplines: Object.keys(PLAB_PASSMEDICINE_DISCIPLINES).length,
    plab_canadaqbank_disciplines: Object.keys(PLAB_CANADAQBANK_DISCIPLINES).length,
