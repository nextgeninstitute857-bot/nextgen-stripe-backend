...
export function buildAylaNotebookDeepLink(input = {}) {
  ...
  if (kind === "library_page") {
    const pageKey = cleanString(input.pageKey || input.page_key, 100).replace(":", "-");
    if (!resourceId || !pageKey) return null;
    const path = `/library/resources/${safeSegment(resourceId)}/pages/${safeSegment(pageKey, 100)}`;
    return {
      surface: "library",
      label: "Return to this page",
      href: withQuery(path, { assignment_id: assignmentId }),
    };
  }
  if (kind === "content_video") {
    ...
    const path = `/content-hub/videos/${safeSegment(resourceId)}`;
    return {
      surface: "content_hub",
      label: seconds ? `Return to ${aylaNotebookTimestampLabel(seconds)}` : "Return to this video",
      href: withQuery(path, { t: seconds || null, assignment_id: assignmentId }),
    };
  }
  if (kind === "qbank_question") {
    ...
    return {
      ...
      href: `/qbank/sessions/${safeSegment(sessionId)}/questions/${safeSegment(questionRef)}`,
    };
  }
  if (kind === "assessment_question") {
    ...
    return {
      ...
      href: withQuery(`/assessments/${safeSegment(assessmentId)}/review`, {
  ...
  if (kind === "roadmap_question") {
    ...
    return {
      ...
      href: `/roadmap/assignments/${safeSegment(assignmentId)}/questions/${safeSegment(questionRef)}`,
    };
  }
  ...
  return {
    ...
    href: withQuery("/revision", { item_id: revisionId }),
  };
}
