...
export function whatsappAiCallingReadiness(env = process.env) {
  ...
  const agentDeployed = truthy(env.LIVEKIT_WHATSAPP_AGENT_DEPLOYED);
  const activationEnabled = truthy(env.WHATSAPP_AI_CALLING_ENABLED);
  const inboundReady = messagingConfigured
    && webhookEnabled
    && openAiConfigured
    && liveKitConfigured
    && agentDeployed
  ...
  if (!openAiConfigured) blockers.push("OpenAI Realtime credentials are missing.");
  if (!liveKitConfigured) blockers.push("LiveKit WhatsApp connector credentials are missing.");
  if (!agentDeployed) blockers.push("The Ayla LiveKit voice worker has not been deployed and verified.");
  ...
  return {
    ...
    livekit_voice_agent_deployed: agentDeployed,
