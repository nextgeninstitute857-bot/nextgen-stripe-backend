...
export function normalizeQuestionTaxonomyReviewManifest(input, allowedSystems) {
  ...
  const items = input.items.map(item => {
    ...
    return {
      ...
      taxonomy: normalizeReviewedQuestionTaxonomy(item.taxonomy, exam === 'nclex' ? shared ? sharedNclexSystems() : aylaNclexDiagnosticSystems(variant) : allowedSystems),
  ...
  return manifest.items.map(item => {
    ...
    if (manifest.exam_track === 'nclex') {
      ...
      if (item.nclex_variants) {
        ...
        normalizeReviewedQuestionTaxonomy(item.taxonomy, variant.shared_allowed_systems);
      } else {
