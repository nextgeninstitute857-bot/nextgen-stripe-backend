...
export function normalizeQuestionTaxonomyReviewManifest(input, allowedSystems) {
  ...
  const items = input.items.map(item => {
    ...
    if (exam === 'nclex' && (!variant || variant !== item.nclex_variant)) fail('NCLEX items require their exact exported nclex_variant');
    ...
    return {
      ...
      ...(exam === 'nclex' ? { nclex_variant: variant } : {}),
      taxonomy: normalizeReviewedQuestionTaxonomy(item.taxonomy, exam === 'nclex' ? aylaNclexDiagnosticSystems(variant) : allowedSystems),
...
function previewRows(manifest, questions) {
  ...
  return manifest.items.map(item => {
    ...
    const existingTaxonomy = Object.fromEntries([...LEVELS.map(level => `${level}_key`), 'labels'].map(key => [key, row.taxonomy?.[key]]));
