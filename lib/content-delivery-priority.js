export const CONTENT_DELIVERY_POLICY_VERSION = "source-year-media-integrity-v3-exam-scoped-fallback";
...
const CONTENT_DELIVERY_VERIFIED_FALLBACK_EXAMS = new Set([
  "plab",
  "amc",
]);
...
function normalizeDeliveryExamTrack(value = "") {
  return String(value || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
...
export function contentDeliverySourceYearsForExam(examTrack = "") {
  const normalizedExamTrack = normalizeDeliveryExamTrack(examTrack);
  return CONTENT_DELIVERY_VERIFIED_FALLBACK_EXAMS.has(normalizedExamTrack)
...
export function contentDeliveryPriorityRank(row = {}, {
  ...
  const sourceYears = contentDeliverySourceYearsForExam(
    examTrack || row.exam_track || row.examTrack,
  );
...
export function contentDeliveryPolicySnapshot({ examTrack = "" } = {}) {
  ...
  const verifiedFallbackEnabled = sourceYears.length > CONTENT_DELIVERY_SOURCE_YEARS.length;
  return {
    version: CONTENT_DELIVERY_POLICY_VERSION,
    source_year_priority: sourceYears,
    fallback_strategy: verifiedFallbackEnabled
      ? "approved_exam_specific_fallback_when_2026_unavailable"
      : "no_prior_year_fallback",
    fallback_exam_scoped: true,
