...
function studentFacingSystem(value = "") {
  const system = clean(value, 120);
  if (!system || /^\d+$/.test(system) || /^(?:system[-_\s]*)?\d+$/i.test(system)) return "";
  return system;
}
...
export function alignAylaPilotStudyDateToRealDate(student = {}, fallback = new Date()) {
  if (student.pilotTest !== true && student.pilot_test !== true) return student;
  const current = aylaPilotStudyDate(student, fallback);
  const realDate = dateOnly(fallback);
  if (!realDate || current.date >= realDate) return student;
  const anchor = new Date(`${current.anchorDate}T12:00:00.000Z`);
  const real = new Date(`${realDate}T12:00:00.000Z`);
  const elapsedDays = Math.max(
    current.dayOffset,
    Math.floor((real.getTime() - anchor.getTime()) / (24 * 60 * 60 * 1000)),
  );
  const dayOffset = Math.min(STEP1_PILOT_MAX_DAYS, elapsedDays);
  if (dayOffset <= current.dayOffset) return student;
  return {
    ...student,
    pilotSimulation: {
      enabled: true,
      anchorDate: current.anchorDate,
      dayOffset,
      simulatedDate: addDays(current.anchorDate, dayOffset),
      maxAdvanceDays: STEP1_PILOT_MAX_DAYS,
    },
  };
}
...
export function buildAylaMateActivityFeed({
  ...
  for (const progress of latest(systemProgress.filter((row) => Number(row?.evidenceCount || 0) > 0), 3)) {
    ...
    const improvement = Math.round((mastery - baseline) * 10) / 10;
    ...
    add({
      ...
      message: `Your verified mastery moved from ${baseline}% to ${mastery}% (${improvement >= 0 ? "+" : ""}${improvement} points).`,
  ...
  for (const attempt of latest(studentRows(questionAttempts, studentId).filter((row) => {
    ...
    add({
      ...
      type: "weakness",
      title: `${studentFacingSystem(attempt.system) || "A weak area"} was added to focused review`,
      message: linked
  ...
  for (const review of latest(studentRows(flashcardReviews, studentId).filter((row) =>
    ...
    add({
      ...
      next: ["again", "hard"].includes(rating)
        ...
        : "Ayla will return this card when its next review becomes due.",
  ...
  for (const event of latest(studentRows(activityHistory, studentId), 3)) {
    ...
    const changeNotice = event.payload?.changeNotice && typeof event.payload.changeNotice === "object"
      ? event.payload.changeNotice
      : null;
    add({
      id: `activity-${event.id}`,
      type: "adaptation",
      title: changeNotice?.title || (event.type === "personal_tutor_recommendation_applied" ? "I applied your roadmap adjustment" : "I rebuilt the future roadmap"),
      message: clean(changeNotice?.message || "The update is saved. Completed history was preserved and only the current or future plan changed.", 500),
      because: clean(changeNotice?.because || event.payload?.reason || event.payload?.recommendationKind || "A recorded Ayla planning action requested the change.", 360),
      next: clean(changeNotice?.next || "The new plan is visible in Today and Roadmap.", 360),
      route: "/dashboard/today",
      evidence: {
        activityId: event.id,
        resultPlanId: event.payload?.resultPlanId || null,
        addedCount: changeNotice?.addedCount || 0,
        movedOrRemovedCount: changeNotice?.movedOrRemovedCount || 0,
        completedHistoryPreserved: changeNotice?.completedHistoryPreserved !== false,
      },
    });
  }
