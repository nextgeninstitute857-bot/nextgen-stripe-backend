...
export function buildAylaMateActivityFeed({
  ...
  for (const attempt of latest(studentRows(questionAttempts, studentId).filter((row) => row?.serverVerified === true && row?.correct === false), 3)) {
  ...
  for (const event of latest(studentRows(activityHistory, studentId), 3)) {
    ...
    add({
      ...
      because: clean(event.payload?.reason || event.payload?.recommendationKind || "A recorded AylaMate planning action requested the change.", 360),
