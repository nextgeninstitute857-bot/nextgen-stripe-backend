...
function knownLeadName(lead = {}) {
  ...
  if (!value || /^(?:doctor|doc|student|lead|unknown|whatsapp user)$/i.test(value) || /^\+?[\d\s().-]+$/.test(value)) return null;
  return value;
}
...
export function createAylaConversationState({ lead = {}, messages = [] } = {}) {
  ...
  return {
    ...
    stage,
    facts: {
      name: cleanNullable(facts.name || knownLeadName(lead), 120),
...
export function evaluateAylaConversationDecision({ decision = {}, state = {}, messages = [] } = {}) {
  ...
  // One focused question is preferred, but a natural two-question turn is a
  // soft style issue and must never silence Ayla after both AI repair passes.
  if ((reply.match(/[?？]/g) || []).length > 2) violations.push("too_many_questions");
  ...
  if (/(?:would you like|do you want|shall i|can i).{0,100}(?:learn more|know more|hear more|explain|tell you more|see how (?:it|this|the (?:programme|program)) works|see (?:the|our) features)/i.test(reply)) {
  ...
  if (
    ...
    && /\b(?:i(?:'ve| have)? signed you up|you(?:'re| are) (?:now )?(?:signed up|enrolled)|i(?:'ve| have)? enrolled you|your (?:seven|7)[ -]day demo is (?:active|activated))\b/i.test(reply)
