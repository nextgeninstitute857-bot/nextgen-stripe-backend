...
function aylaMedOperationalFacts(value = {}) {
  ...
  const prospect = scoped && value.identity_scope === "whatsapp_prospect";
  return {
    verified: Boolean(scoped && !prospect),
    paid: prospect ? null : scoped && value.paid === true,
    demo_status: prospect ? "unknown" : validEnum(demo.status, ["unknown", "not_issued", "issued", "active", "expired", "failed"], "unknown"),
    email_delivery_status: prospect ? "unknown" : validEnum(demo.email_delivery_status, ["accepted", "delivered", "failed", "unknown"], "unknown"),
    expires_at: !prospect && typeof demo.expires_at === "string" && Number.isFinite(Date.parse(demo.expires_at))
      ? new Date(demo.expires_at).toISOString() : null,
    prospect_verified: Boolean(prospect && value.prospect?.sender_verified === true),
...
- prospect_verified only verifies the incoming WhatsApp sender and configured receiving asset. It does not verify ownership of any email or an existing account; paid=null is unknown, not unpaid. Keep such conversations to qualification and general product explanations. Never disclose whether an address has an account or link it to this chat. The deterministic private-demo controller, not a model action or memory patch, decides whether a new-address invitation is authorized.
