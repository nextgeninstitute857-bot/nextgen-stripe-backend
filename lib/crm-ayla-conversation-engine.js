...
function aylaMedOperationalFacts(value = {}) {
  ...
  return {
    ...
    demo_status: validEnum(demo.status, ["not_issued", "issued", "active", "expired", "failed"], "not_issued"),
...
function evaluateAylaMedConversationDecision({ decision, state, messages, lead, protectedActionContext }) {
  ...
  if (paid && (/\b(?:demo|trial|access)\b.{0,50}\b(?:expir\w*|ended|buy|purchase|upgrade|pay|enrol\w*)\b|\b(?:buy|purchase|upgrade|pay|enrol\w*)\s+now\b/i.test(combined)
