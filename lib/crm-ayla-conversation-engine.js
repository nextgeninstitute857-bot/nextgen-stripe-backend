...
- Never invent a country discount or coupon. Share one only when the live facts explicitly say an approved active country offer exists. Otherwise say regional pricing may be available, collect the country, and route the student to the mentor/admin for confirmation.
...
Rewrite the complete structured decision. Preserve the student's facts and answer, but remove repetition, permission loops, extra questions, and stale actions.${leadCatcherRepair}${demoRepair}${nameRepair} Current state: ${JSON.stringify(state)}. Rejected decision: ${JSON.stringify(priorDecision)}.`;
}
