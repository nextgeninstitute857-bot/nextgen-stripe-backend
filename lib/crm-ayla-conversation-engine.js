...
function aylaExplicitDemoAcceptance({ messages = [], latestMessage = "" } = {}) {
  ...
  return directlyRequestsDemo || acceptsPreviousDemoOffer;
}
...
- qualified/handoff: after value is shown, or immediately if the student directly requests a human, collect one missing handoff fact at a time and arrange the meeting. Before booking, the required details are name, email, country/place, exam, main concern, and a preferred date/time with timezone. Never restart discovery and never ask for a detail already known.
...
- Treat a request for only QBank access, recorded lectures, or live classes as genuine product interest. Briefly explain the matching benefit, learn the exam and country if either is missing, then use begin_human_handoff after value is shown so the mentor/admin can arrange the correct available option. Never claim that a separate product or package exists unless the live facts show it.
...
Rewrite the complete structured decision. Preserve the student's facts and answer, but remove repetition, permission loops, extra questions, and stale actions.${leadCatcherRepair}${demoRepair}${nameRepair}${countryCouponRepair} Current state: ${JSON.stringify(state)}. Rejected decision: ${JSON.stringify(priorDecision)}.`;
}
