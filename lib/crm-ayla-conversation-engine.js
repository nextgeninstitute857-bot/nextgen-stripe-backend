...
export function createAylaConversationState({ lead = {}, messages = [] } = {}) {
  ...
  const leadCountry = lead.country_confirmed === true || confirmedCountrySources.has(leadCountrySource)
    ...
    : null;
  const confirmedCountry = cleanNullable(storedCountry || leadCountry, 120);
...
function aylaExplicitDemoAcceptance({ messages = [], latestMessage = "" } = {}) {
  ...
  if (!latestStudentText || /\b(?:do not|don't|dont|not now|no thanks?|decline|skip)\b/i.test(latestStudentText)) return false;

  const lastAssistantText = cleanText(
    [...normalizedMessages].reverse().find((item) => item.role === "assistant")?.text || "",
    1000,
  );
  const directlyRequestsDemo = /\b(?:demo|trial)\b/i.test(latestStudentText)
    && /\b(?:yes|want|try|take|start|send|share|open|join|access|link|please|before paying|interested)\b/i.test(latestStudentText);
  const requestsRealLmsPreview = /\b(?:see|view|show|try|open|access|look at|take a look at)\b.{0,60}\b(?:lms|platform|dashboard|system)\b|\b(?:lms|platform|dashboard)\b.{0,60}\b(?:see|view|show|try|open|access|look)\b/i.test(latestStudentText);
...
export function aylaExplicitHumanHandoffRequest(text = "") {
  ...
  return [
    ...
    /\b(?:i\s+(?:want|need|would\s+like)|can\s+i\s+(?:have|get))\b.{0,45}\b(?:human|mentor|admin|advisor|adviser|consultation|meeting|google\s*meet|phone\s+call)\b/i,
...
export function normalizeAylaConversationDecision(raw = {}, state = {}, context = {}) {
  ...
  if (
    ...
    && !(action === "send_demo" && explicitDemoAcceptance)
  ) {
    normalized.action = "reply_only";
    normalized.media_keys = [];
  }
...
export function evaluateAylaConversationDecision({ decision = {}, state = {}, messages = [] } = {}) {
  ...
  const asksToSeeLms = /\b(?:see|view|show|try|open|access|look at|take a look at)\b.{0,60}\b(?:lms|platform|dashboard|system)\b|\b(?:lms|platform|dashboard)\b.{0,60}\b(?:see|view|show|try|open|access|look)\b/i.test(latestStudentText);
  ...
  if (
    ...
    && !(decision.action === "send_demo" && explicitDemoAcceptance)
  ) {
  ...
  if (
    /(?:feel free to\b|let me know if (?:you have|there are) any|do not hesitate to (?:reach out|ask)|i(?:'m| am) here if you (?:need|want|have))\b/i.test(combinedReply)
  ...
  const studentExplicitlyRequestedHandoff = aylaExplicitHumanHandoffRequest(latestStudentText);
...
export function applyAylaConversationDecision({ state = {}, decision = {}, responseId = null, now = new Date().toISOString() } = {}) {
  ...
  if (patch.country) nextFactSources.country = "conversation_self_reported";
...
- new: greet naturally once and learn the person's name as the first easy question. If they already give their exam or situation, acknowledge it first and then ask their name instead of ignoring what they said.
...
- value_tour: for a prospective student, as soon as the student's name and exam are known after the first exchange, confidently introduce the programme and choose send_feature_tour once. Do not wait for them to repeat that they are interested and do not ask permission to explain. If their main problem is known, personalise the introduction to it; otherwise connect the programme to organised preparation for their exam. The backend then sends five separate approved feature pictures/captions. Your reply is the personalised introduction, and follow_up is the natural demo/live-or-recording closing sent after the cards.
...
- Never repeat a question, feature tour, recording, or live invitation already present in state.completed_actions or state.asked_fields. The one exception is a direct demo acceptance/request: always send the demo link in that confirming reply so the student does not need to search the chat.
...
- A phone calling code is only a location hint, never proof of country. If the student has not clearly stated their current country and no explicit form/admin country is confirmed, ask naturally where they are currently based. Save their stated answer in memory_patch.country and never ask again once confirmed.
...
- send_feature_tour: only once when a prospective student's name + exam are clear after the first exchange, or when the student explicitly asks for the complete feature overview. A known main need improves personalisation but is not required. Set all five media_keys and provide follow_up.
...
export function buildAylaConversationRepairPrompt({ violations = [], priorDecision = {}, state = {} } = {}) {
  const leadCatcherRepair = violations.includes("prospective_lead_not_advanced_to_feature_tour")
    ? " The prospective student's name and exam are already known, so choose send_feature_tour now. Write a warm tailored introduction without a question; the backend will send the five separate feature cards and then your demo/live-or-recording follow-up."
    : "";
  ...
  const countryOfferDetailRepair = violations.some((item) => ["approved_country_discount_not_stated", "approved_country_offer_not_marked_one_time", "approved_country_offer_expiry_not_stated"].includes(item))
    ...
    : "";
  const liveClassRepair = violations.includes("live_class_preview_missing_current_session")
    ? " The student asked to attend a class and the current live facts contain a session. Include its exact title, time and student join link while still answering the LMS-preview request."
    : "";
  return `Your proposed decision failed these conversation-quality checks: ${violations.join(", ")}.
Rewrite the complete structured decision. Preserve the student's facts and answer, but remove repetition, permission loops, extra questions, and stale actions.${leadCatcherRepair}${demoRepair}${nameRepair}${countryCouponRepair}${handoffRepair}${demoResendRepair}${repeatedDemoRepair}${handbackRepair}${lmsPreviewRepair}${incompleteRepair}${unexpectedFollowUpRepair}${countryOfferDetailRepair}${liveClassRepair} Current state: ${JSON.stringify(state)}. Rejected decision: ${JSON.stringify(priorDecision)}.`;
}
