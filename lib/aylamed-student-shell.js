...
function profileRequiresDiagnostic(profile = {}) {
  const onboardingPath = cleanString(profile.onboardingPath || profile.onboarding_path || "", 80).toLowerCase();
  const onboardingStatus = cleanString(profile.onboardingStatus || profile.onboarding_status || "", 80).toLowerCase();
  return onboardingPath === "diagnostic_test"
    && profile.serverVerifiedBaseline !== true
    && profile.server_verified_baseline !== true
    && onboardingStatus !== "complete";
}

function profileRequiresStartingChoice(profile = {}) {
  const onboardingPath = cleanString(profile.onboardingPath || profile.onboarding_path || "", 80).toLowerCase();
  const onboardingStatus = cleanString(profile.onboardingStatus || profile.onboarding_status || "", 80).toLowerCase();
  return onboardingPath === "starting_choice_pending" || onboardingStatus === "setup_pending";
}
...
export function resolveAylaStudentShell({
  ...
  for (const examTrackId of candidateTracks) {
    ...
    const examVariant = examTrackId === "nclex"
      ...
      : "";
    const diagnosticRequired = Boolean(profile && profileRequiresDiagnostic(profile));
    const startingChoiceRequired = Boolean(profile && profileRequiresStartingChoice(profile));
    const onboardingRequired = diagnosticRequired
      ? {
        required: true,
        type: "baseline_diagnostic",
        route: `/diagnostic?exam=${encodeURIComponent(examTrackId)}`,
        reason: "A verified starting diagnostic is required before AylaMed personalizes videos, questions, revision and the study roadmap.",
      }
      : startingChoiceRequired
        ? {
          required: true,
          type: "starting_choice",
          route: `/diagnostic?exam=${encodeURIComponent(examTrackId)}`,
          reason: "Set your study details and choose diagnostic, quick profile, or starting fresh before opening the adaptive dashboard.",
        }
        : null;
    dashboards.push({
      id: examTrackId,
      ...
      profile_status: profile ? (startingChoiceRequired ? "setup_required" : diagnosticRequired ? "diagnostic_required" : "ready") : "setup_required",
