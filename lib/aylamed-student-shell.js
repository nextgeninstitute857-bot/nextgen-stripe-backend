...
function featureAccess(enrollment = {}, plan = {}) {
  const fullAccess = enrollment.is_full_access === true || plan?.is_full_access === true;
  const configured = new Set([
    ...cleanList(enrollment.included_features),
    ...cleanList(enrollment.features),
    ...cleanList(plan?.included_features),
    ...cleanList(plan?.features),
  ].map(normalizeFeature).filter(Boolean));
  const enabled = AYLA_STUDENT_FEATURES.filter((feature) => fullAccess || feature.aliases.some((alias) => configured.has(alias))).map((feature) => feature.key);
