...
function matchingExamSystems(requested = [], examDefinition = {}) {
  ...
  const matched = [];
  const unknown = [];
  for (const input of cleanList(requested, 100)) {
...
export function aylaVerifiedDiagnosticPlanCanYield({
  plan = null,
  assignments = [],
  student = {},
  date = "",
  ...
  if (!plan || student.serverVerifiedBaseline !== true) return false;
  if (today && String(date || plan.date || "") < String(today)) return false;
  const rows = Array.isArray(assignments) ? assignments.filter(Boolean) : [];
  if (!rows.length) return false;
  return rows.every((row) => ["diagnostic", "baseline_diagnostic"]
    .includes(String(row.category || row.type || "").trim().toLowerCase()));
}
...
function questionTaxonomyValue(question = {}, keys = []) {
  const taxonomy = question.taxonomy && typeof question.taxonomy === "object" ? question.taxonomy : {};
  const labels = taxonomy.labels && typeof taxonomy.labels === "object" ? taxonomy.labels : {};
  for (const key of keys) {
    const value = [
      ...
      taxonomy[key],
      labels[key],
    ].map((item) => cleanText(item, 220)).find(Boolean);
    if (value) return value;
  }
  return "";
}

function diagnosticQuestionEvidenceProfile(question = {}, storedProfile = {}, examDefinition = {}) {
  const stored = storedProfile && typeof storedProfile === "object" ? storedProfile : {};
  const system = questionSystem({
    ...question,
    system_key: stored.system || question.system_key,
  }, examDefinition);
  return {
    system,
    subsystem: cleanText(
      stored.subsystemLabel
      || stored.subsystem_label
      || questionTaxonomyValue(question, ["subsystem_key", "subsystemKey", "subsystem"])
      || stored.subsystem,
      220,
    ),
    topic: cleanText(
      stored.topicLabel
      || stored.topic_label
      || questionTaxonomyValue(question, ["topic_key", "topicKey", "topic"])
      || stored.topic
      || question.title,
      220,
    ),
    subtopic: cleanText(
      stored.subtopicLabel
      || stored.subtopic_label
      || questionTaxonomyValue(question, ["subtopic_key", "subtopicKey", "subtopic"])
      || stored.subtopic,
      220,
    ),
  };
...
function diagnosticEvidencePath(profile = {}, level = "topic") {
  if (!profile.system) return "";
  if (level === "subsystem") {
    return profile.subsystem ? `${profile.system} > ${profile.subsystem}` : "";
  }
  if (level === "topic") {
    return profile.topic
      ? [profile.system, profile.subsystem, profile.topic].filter(Boolean).join(" > ")
      : "";
  }
  if (level === "subtopic") {
    return profile.subtopic
      ? [profile.system, profile.subsystem, profile.topic, profile.subtopic]
          .filter(Boolean)
          .join(" > ")
      : "";
  }
...
function addDiagnosticEvidenceRow(rows, level, profile, answer) {
  const key = diagnosticEvidencePath(profile, level);
  if (!key) return;
  const row = rows.get(key) || {
    key,
    level,
    system: profile.system,
    subsystem: level === "subsystem" || level === "topic" || level === "subtopic"
      ? profile.subsystem || null
      : null,
    topic: level === "topic" || level === "subtopic" ? profile.topic || null : null,
    subtopic: level === "subtopic" ? profile.subtopic || null : null,
    correct: 0,
    answered: 0,
    total: 0,
  };
...
function finalizedDiagnosticEvidence(rows, timestamp) {
  return [...rows.values()]
    .map((row) => ({
      ...row,
      ...
      evidenceScope: "sampled_questions_only",
      confidence: row.total >= 3
        ? "multi_question_signal"
        : row.total === 2
          ? "limited_sample"
          : "single_question_signal",
    }))
    .sort((left, right) => (
...
function evidenceRowsByKey(rows = []) {
  return Object.fromEntries(rows.map((row) => [row.key, row]));
}

function weakDiagnosticSignals(rows = [], limit = 12) {
  return rows
...
export function buildAylaVerifiedDiagnosticBaseline({
  ...
  const rows = new Map();
  const subsystemRows = new Map();
  const topicRows = new Map();
  const subtopicRows = new Map();
  const storedTaxonomy = session.diagnosticTaxonomyByQuestionId
    && typeof session.diagnosticTaxonomyByQuestionId === "object"
    ? session.diagnosticTaxonomyByQuestionId
    : {};

  for (const mapping of Array.isArray(session.questions) ? session.questions : []) {
    const questionId = String(mapping.contentQuestionId || mapping.content_question_id);
    const question = questionsById.get(questionId);
    const profile = diagnosticQuestionEvidenceProfile(
      question || {},
      storedTaxonomy[questionId],
      examDefinition,
    );
    ...
    if (answer?.correct === true) row.correct += 1;
    rows.set(system, row);
    addDiagnosticEvidenceRow(subsystemRows, "subsystem", profile, answer);
    addDiagnosticEvidenceRow(topicRows, "topic", profile, answer);
    addDiagnosticEvidenceRow(subtopicRows, "subtopic", profile, answer);
  }
  ...
  const subsystemEvidence = finalizedDiagnosticEvidence(subsystemRows, timestamp);
  const topicEvidence = finalizedDiagnosticEvidence(topicRows, timestamp);
  const subtopicEvidence = finalizedDiagnosticEvidence(subtopicRows, timestamp);

  return {
    currentScore: Math.max(0, Math.min(100, Number(session.scorePercent || 0))),
    systemBaselines,
    subsystemBaselines: evidenceRowsByKey(subsystemEvidence),
    topicBaselines: evidenceRowsByKey(topicEvidence),
    subtopicBaselines: evidenceRowsByKey(subtopicEvidence),
    weakAreas: ordered.filter((row) => row.score < 70).slice(0, 6).map((row) => row.system),
    weakSubsystemSignals: weakDiagnosticSignals(subsystemEvidence),
    weakTopicSignals: weakDiagnosticSignals(topicEvidence),
    weakSubtopicSignals: weakDiagnosticSignals(subtopicEvidence),
    diagnosticTaxonomyEvidence: {
      version: 1,
      sampledOnly: true,
      untestedTaxonomyIsUnknown: true,
      systems: ordered.length,
      subsystems: subsystemEvidence.length,
      topics: topicEvidence.length,
      subtopics: subtopicEvidence.length,
      subsystemEvidence,
      topicEvidence,
      subtopicEvidence,
    },
    ...
    diagnosticCoverage: {
      ...
      subsystemsSampled: subsystemEvidence.length,
      topicsSampled: topicEvidence.length,
      subtopicsSampled: subtopicEvidence.length,
      sampledTaxonomyOnly: true,
      allTopicsTested: false,
      untestedTaxonomyIsUnknown: true,
    },
  };
}
