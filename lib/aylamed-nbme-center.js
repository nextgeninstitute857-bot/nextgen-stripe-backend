...
export function assertAylaNbmeReviewedReleaseForm(formId = "", collectionKey = "") {
  ...
  if (!AYLA_NBME_STEP1_REVIEWED_RELEASE_FORM_IDS.includes(cleanFormId)) {
    throw nbmeError(
  ...
  if (!definition || definition.formId !== cleanFormId || definition.examTrack !== "usmle-step-1") {
    throw nbmeError(
      "The requested form does not match its reviewed Step 1 collection",
      409,
...
export function buildAylaNbmeReadinessSnapshot({
  ...
  if (inProgress) {
    ...
    recommendation = {
      state: "baseline_due",
      title: "Take a baseline self-assessment",
      reason: Number.isFinite(daysToTarget)
        ? `A verified baseline will help the same roadmap adapt the remaining ${Math.max(0, daysToTarget)} days.`
        : "A verified baseline will help the same roadmap use your actual readiness trend instead of assumptions.",
