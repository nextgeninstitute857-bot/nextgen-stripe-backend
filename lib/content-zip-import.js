...
export async function previewUniversalQuestionZip({
  ...
  for (const pair of pairs) {
    ...
    await readPreviewArray(questionPath, "previewing_questions", async (question) => {
      ...
      const isControlledSourceTaxonomy = isAylaOwned
        || isStep1SourceTaxonomy
        || row.sourceData?.source_taxonomy_adapter === "multi_exam_source_taxonomy_v1";
      ...
      if (validation.length) {
        ...
        if (isControlledSourceTaxonomy) counts.taxonomy_ledger_incomplete += 1;
        const blockingReasons = validation.filter((reason) => (
          isControlledSourceTaxonomy || BLOCKING_VALIDATION_ERRORS.has(reason)
        ));
      ...
      if (isControlledSourceTaxonomy && !isAylaOwned && !isStep1SourceTaxonomy) {
        counts.multi_exam_source_taxonomy_questions = Number(
          counts.multi_exam_source_taxonomy_questions || 0,
        ) + 1;
        counts.taxonomy_ledger_complete += 1;
      }
