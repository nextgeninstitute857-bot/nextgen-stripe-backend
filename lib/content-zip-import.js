...
export async function importUniversalQuestionZip({ inventory, job, onBatch, batchSize = 100 }) {
  const pairs = pairQuestionAnswerFiles(inventory.names);
  const totals = { collections: 0, questions_seen: 0, valid_questions: 0, quarantined: 0, created: 0, reused: 0, aliases_created: 0, source_duplicates: 0, answers: 0 };
  const errors = [];
  for (const pair of pairs) {
    if (!pair.answerFile) { errors.push({ collection: pair.collectionKey, error: "missing_answers_file" }); continue; }
    const questionPath = inventory.extractedJson.get(pair.questionFile);
