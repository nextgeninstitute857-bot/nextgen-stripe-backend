...
export async function previewUniversalQuestionZip({
  ...
  const counts = {
    ...
    aylamed_owned_questions: 0, taxonomy_ledger_complete: 0, taxonomy_ledger_incomplete: 0,
  ...
  for (const pair of pairs) {
    ...
    await readJsonArray(questionPath, async (question) => {
      ...
      if (validation.length) {
        ...
        if (isAylaOwned) counts.taxonomy_ledger_incomplete += 1;
        const blockingReasons = validation.filter((reason) => isAylaOwned || BLOCKING_VALIDATION_ERRORS.has(reason));
  ...
  counts.taxonomy_coverage_percent = counts.aylamed_owned_questions || counts.taxonomy_ledger_incomplete
  ...
  return {
    ...
    taxonomyLedger: aylaMedTaxonomyLedgerSummary(),
  };
}
